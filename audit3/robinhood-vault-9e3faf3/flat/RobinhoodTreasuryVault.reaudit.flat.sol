...
library VaultBDepositLib {
    ...
    function _previewRedeemCycleSettlement(
        ...
        uint256 maximumNavLoss = Math.mulDiv(assetsSnapshot, maximumLossBps, 10_000);
        if (assetsSnapshot - capAssets > maximumNavLoss) return (false, 0);
    ...
    function _initializeRedeemCycleSettlement(
        ...
        uint256 capAssets = assetsSnapshot < currentAssets ? assetsSnapshot : currentAssets;
        uint256 maximumNavLoss = Math.mulDiv(assetsSnapshot, maximumLossBps, 10_000);
        uint256 navLoss = assetsSnapshot - capAssets;
        if (navLoss > maximumNavLoss) {
            revert RedeemCycleExecutionLossExceeded(navLoss, maximumNavLoss, navLoss - maximumNavLoss);
        }
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// self-administered so the default admin cannot silently collapse the intended
    /// guardian/admin separation by granting itself emergency authority.
    ...
    function commitRedeemCycle() external whenNotPaused nonReentrant {
        ...
        // before the strategy crossed its one-way commit boundary. If an external
        // commitment witness exists without that local snapshot, record only a
        // zero recovery marker. It starts the timeout clock but can never turn a
        // caller-timed post-close NAV into a positive payout basis.
        if (activeStrategy.withdrawalCycleCommitted()) {
            _writeRedeemCycleSnapshot(0, 0);
            return;
        }
...
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    ...
    function commitThresholdShares() public view override returns (uint256 threshold) {
        if (outstandingRedeemCount == 0) return 0;
        return VaultBDepositLib.redeemCommitThreshold(totalSupply(), 0, MIN_REDEEM_SHARES, ROBINHOOD_BATCH_COMMIT_BPS);
    }

    function _redeemSeatFloorDivisor() internal view override returns (uint256) {
        return maxPendingRedeems;
    }

    function _redeemCycleSnapshotAssets() internal view override returns (uint256) {
        return totalAssetsUpper();
    }
}
