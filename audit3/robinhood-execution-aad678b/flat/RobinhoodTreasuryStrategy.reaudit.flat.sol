...
contract RobinhoodPriceGuard {
    ...
    function exitPrices(RobinhoodMarket market) public view returns (Prices memory p) {
        return _exitPrices(market, maxSpotTwapDeviationBps);
    }

    /// @notice Emergency exits deliberately do not require TWAP availability
    /// or spot/TWAP convergence. The pinned pool spot is always available and
    /// is tightened (never raised) by any live TWAP/oracle reference. Caller
    /// floors and the emergency slippage cap remain independently enforced.
    function emergencyExitPrices(RobinhoodMarket market) public view returns (Prices memory p) {
        ...
        p.twapUsdGPerRisk = p.spotUsdGPerRisk;

        try this.twapSqrtPriceX96(market) returns (uint160 twapSqrt) {
            uint256 twapPrice = _quoteRiskToUsdG(market, twapSqrt, ONE_RISK_TOKEN);
            if (twapPrice < p.twapUsdGPerRisk) p.twapUsdGPerRisk = twapPrice;
        } catch {}
        try this.oraclePrice(market) returns (uint256 oraclePrice_) {
            p.oracleUsdGPerRisk = oraclePrice_;
            if (oraclePrice_ < p.twapUsdGPerRisk) p.twapUsdGPerRisk = oraclePrice_;
        } catch {}
    ...
    function lowerRiskValue(RobinhoodMarket market, uint256 riskAmount) public view returns (uint256) {
        ...
        try this.exitPrices(market) returns (Prices memory normal) {
            ...
            p = emergencyExitPrices(market);
        }
        uint256 referencePrice = p.spotUsdGPerRisk < p.twapUsdGPerRisk ? p.spotUsdGPerRisk : p.twapUsdGPerRisk;
        try this.oraclePrice(market) returns (uint256 oraclePrice_) {
    ...
    function minimumOut(
        ...
        if (tokenIn == risk && tokenOut == USDG) {
            ...
            uint256 riskPrice = p.spotUsdGPerRisk < p.twapUsdGPerRisk ? p.spotUsdGPerRisk : p.twapUsdGPerRisk;
...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    {
        if (shares == 0 || shares > shareBalance() || minAssetsOut == 0) revert InvalidAmount();
        if (block.timestamp > deadline) revert DeadlineExpired(deadline);
...
library RobinhoodVenueLib {
    ...
    function closeReference(
        ...
        uint256 trackedRisk,
        uint256 trackedStable
    ) external view returns (PositionData memory p, uint256 expected0, uint256 expected1, uint256 referenceTotal) {
        p = _position(manager, pool, poolFee, tokenId);
        (expected0, expected1) = LiquidityAmounts.getAmountsForLiquidity(
            referenceSqrt,
        ...
        uint256 positionRisk = riskToken0 ? expected0 + fees0 : expected1 + fees1;
        uint256 positionStable = riskToken0 ? expected1 + fees1 : expected0 + fees0;
        referenceTotal =
            trackedStable + positionStable + FullMath.mulDiv(trackedRisk + positionRisk, referenceRiskPrice, 1e18);
    }

    function totalCloseFloor(uint256 referenceTotal, uint256 suppliedTotal, uint16 slippage, bool emergency)
        external
        pure
        returns (uint256 totalFloor)
    {
        uint256 requiredTotal = FullMath.mulDiv(referenceTotal, BPS - slippage, BPS);
        if (emergency && suppliedTotal < requiredTotal) {
            revert UnsafeExecutionFloor(suppliedTotal, requiredTotal);
        }
        totalFloor = suppliedTotal < requiredTotal ? requiredTotal : suppliedTotal;
    }

    function validateDeployedValue(
        uint256 allocationAssets,
        uint256 riskAmount,
        uint256 stableAmount,
        uint256 twapUsdGPerRisk,
        uint256 oneRiskToken,
        uint16 slippage
    ) external pure {
        uint256 deployedValue =
            stableAmount + FullMath.mulDiv(riskAmount, twapUsdGPerRisk, oneRiskToken);
        uint256 minimum = FullMath.mulDiv(allocationAssets, BPS - slippage, BPS);
        if (deployedValue < minimum) revert UnsafeExecutionFloor(deployedValue, minimum);
    }
    ...
    function executeClose(
        ...
        if (riskToSell != 0) {
            ...
            if (fullRequiredFloor == 0) {
                ...
                if (swapOutput < requiredFloor) revert UnsafeExecutionFloor(swapOutput, requiredFloor);
...
contract BoundedUniswapV3Venue is ReentrancyGuard, IERC721Receiver {
    ...
    uint256 internal constant WETH_TERMINAL_DUST = 1e9;
    uint256 internal constant NVDA_TERMINAL_DUST = 1e10;
    ...
    constructor(
        ...
        if (
            POSITION_MANAGER.code.length == 0 || ROUTER.code.length == 0
                ...
                // One V3 tick is approximately one basis point. The router's
                // per-swap movement bound must never be looser than the
                // independently enforced spot/TWAP corridor.
                // forge-lint: disable-next-line(unsafe-typecast)
                || uint24(maxSwapTickMovement_) > guard_.maxSpotTwapDeviationBps()
        ) revert InvalidDeployment();
        initializer = initializer_;
        guard = guard_;
    ...
    function close(CloseParams calldata p) external onlyController nonReentrant returns (uint256 assetsReturned) {
        ...
        if (activePositionBurned) {
            ...
            referenceTotal =
                activeUsdGInventory + FullMath.mulDiv(activeRiskInventory, prices.twapUsdGPerRisk, ONE_RISK_TOKEN);
        } else {
        ...
        uint256 observedValue = assetsReturned + FullMath.mulDiv(riskRemaining, prices.twapUsdGPerRisk, ONE_RISK_TOKEN);
        if (observedValue < totalFloor) revert CumulativeExecutionLoss(observedValue, totalFloor);
        bytes32 jobId = activeJobId;
        if (assetsReturned != 0) usdg.safeTransfer(controller, assetsReturned);
        if (riskRemaining != 0) {
            uint256 remainingFloor = FullMath.mulDiv(
                FullMath.mulDiv(riskRemaining, prices.twapUsdGPerRisk, ONE_RISK_TOKEN), BPS - slippage, BPS
            );
            uint256 terminalDust = market == RobinhoodMarket.ETH ? WETH_TERMINAL_DUST : NVDA_TERMINAL_DUST;
            if (remainingFloor != 0 || riskRemaining > terminalDust) {
    ...
    function estimatedValueExecution() external view returns (uint256) {
        return _estimatedValue(2);
    }

    function _estimatedValue(uint8 mode) internal view returns (uint256 value) {
        RobinhoodMarket market = activeMarket;
        if (market == RobinhoodMarket.NONE) return 0;
        value = activeUsdGInventory;
        uint256 totalRisk = activeRiskInventory;
        RobinhoodPriceGuard.Prices memory prices = mode == 1
            ? guard.healthyPrices(market)
            : (mode == 2 ? guard.executionPrices(market) : guard.emergencyExitPrices(market));
        uint256 tokenId = activeTokenId;
        if (tokenId != 0 && !activePositionBurned) {
            uint160 referenceSqrt = TickMath.getSqrtRatioAtTick(prices.twapTick);
        ...
        if (totalRisk != 0) {
            if (mode == 1) {
                value += guard.upperRiskValue(market, totalRisk);
            } else if (mode == 2) {
                value += FullMath.mulDiv(totalRisk, prices.twapUsdGPerRisk, ONE_RISK_TOKEN);
            } else {
                value += guard.lowerRiskValue(market, totalRisk);
            }
        }
    }
...
/// @notice Storage-independent linked capital movement for the Robinhood
/// Strategy. Calls execute in Strategy context, so immutable child contracts
/// still observe only the bound Strategy as controller and recipient.
library RobinhoodStrategyLib {
    ...
    function chargeableLoss(uint256 executionLoss, uint256 feeBefore, uint256 feeAfter)
        external
        pure
        returns (uint256)
    {
        uint256 feeOffset = feeBefore > feeAfter ? feeBefore - feeAfter : 0;
        return executionLoss > feeOffset ? executionLoss - feeOffset : 0;
    }
    ...
    function payFee(IERC20 asset, address recipient, uint256 amount) external returns (uint256 paid) {
        ...
        try IFeeSink(recipient).recordFee(amount) {
            ...
            paid = beforeBalance - asset.balanceOf(address(this));
    ...
    function redeemAll(address morphoAddress, uint256 deadline, bool emergency) external returns (uint256 released) {
        ...
        uint256 preview = morpho.previewAssets();
        if (preview == 0) revert StrategyUnavailable();
        ...
        returns (uint256)
    {
        ...
        return asset.balanceOf(address(this)) + morpho.previewAssets() + venueAssets;
    }
    ...
    function grossAssetsExecution(IERC20 asset, address morphoAddress, address venueAddress)
        ...
        returns (uint256)
    {
        return asset.balanceOf(address(this)) + IRobinhoodMorphoCapital(morphoAddress).previewAssets()
...
contract RobinhoodTreasuryStrategy is IVaultBAsyncStrategy, AccessControlDefaultAdminRules, ReentrancyGuard {
        ...
        returns (uint256 tokenId)
    {
        ...
        uint256 allocationAssets = Math.mulDiv(gross, accepted.allocationBps, BPS);
        uint256 maximum = Math.mulDiv(gross, maxLpAllocationBps, BPS);
        ...
        returns (uint256 assetsRecovered)
    {
        ...
        bool exiting = state == StrategyState.EXITING;
        bool halted = state == StrategyState.HALTED || (exiting && _exitReturnsToHalted);
        if (!halted && !exiting && state != StrategyState.LP_ACTIVE) {
            revert InvalidState(StrategyState.LP_ACTIVE, state);
        }
        if (venue.activeTokenId() == 0) revert StrategyUnavailable();

        RobinhoodMarket market = activeMarket;
        if (
            market == RobinhoodMarket.NONE || activeTokenId == 0 || venue.activeMarket() != market
                || venue.activeTokenId() != activeTokenId
        ) revert StrategyUnavailable();
        if (p.emergency && !exiting) {
            if (!halted) revert InvalidState(StrategyState.HALTED, state);
            bool healthy;
            // A closed equity session or stale/paused oracle is not itself an
            // emergency: ordinary exit remains TWAP-bounded and available.
            // The widened path is admitted only when the normal spot/TWAP
            // corridor itself is unavailable.
            try priceGuard.exitPrices(market) returns (RobinhoodPriceGuard.Prices memory) {
                healthy = true;
            } catch {}
            if (healthy) revert EmergencyNotRequired();
        }
        // Validate emergency admission before invoking the Vault callback. A
        // genuine execution outage must not be blocked by an unrelated lower-
        // NAV read performed while materializing a queued withdrawal cycle.
        if (liveWithdrawalCount != 0 && !cycleCommitted) {
            IRobinhoodVaultView(vault).prepareRedeemCycleCommit();
            _commitCycle();
        }
        bytes32 jobId = activeJobId;
        if (!exiting) {
            _exitGrossBefore = estimatedGrossAssetsExecution();
            _exitReturnsToHalted = halted || p.emergency;
            state = StrategyState.EXITING;
        }
        uint256 beforeBalance = asset.balanceOf(address(this));
        assetsRecovered = venue.close(p);
        uint256 observed = asset.balanceOf(address(this)) - beforeBalance;
        if (observed != assetsRecovered) revert TransferMismatch(assetsRecovered, observed);

        if (venue.activeTokenId() != 0) {
            return assetsRecovered;
        }

        uint256 grossAfter = estimatedGrossAssetsLower();
        if (cycleCommitted && _exitGrossBefore > grossAfter) {
            cycleExecutionLoss = _exitGrossBefore - grossAfter;
            (, uint256 feeAfter) =
                RobinhoodStrategyLib.pendingFee(grossAfter, unremittedFee, accountedAssets, performanceFeeBps);
            cycleFeeBefore = RobinhoodStrategyLib.chargeableLoss(cycleExecutionLoss, cycleFeeBefore, feeAfter);
        }
        ...
        returns (uint256 assetsReleased)
    {
        ...
        uint256 shares = morphoAdapter.shareBalance();
        if (shares != 0) {
            uint256 protectedFloor = morphoAdapter.previewAssets() * 9_995 / BPS;
            if (minAssetsOut < protectedFloor) minAssetsOut = protectedFloor;
            assetsReleased = morphoAdapter.redeem(shares, minAssetsOut, deadline, true);
            emit CapitalReleased(shares, assetsReleased, true);
        }
        emit StrategyHalted(msg.sender);
    }
    ...
    function withdrawToVault(uint256 assetsNeeded) external onlyVault nonReentrant returns (uint256 withdrawn) {
        if (venue.activeTokenId() != 0 || cycleCommitted) revert WithdrawalNotReady();
        _crystallizeFee();
        ...
        _reduceAccountedAssets(withdrawn);
    ...
    function managerWithdrawAll() external onlyRole(KEEPER_ROLE) nonReentrant returns (uint256 withdrawn) {
        if (state == StrategyState.HALTED) revert InvalidState(StrategyState.MORPHO_IDLE, state);
        if (venue.activeTokenId() != 0 || liveWithdrawalCount != 0) return 0;
        _crystallizeFee();
        _redeemAllMorpho(block.timestamp, false);
        withdrawn = asset.balanceOf(address(this)) - unremittedFee;
        ...
        returns (uint256 withdrawn)
    {
        ...
        if (assetsNeeded == 0) revert InvalidAmount();
        if (!cycleCommitted) revert CycleNotCommitted();
        if (venue.activeTokenId() != 0) revert WithdrawalNotReady();
        _crystallizeFee();
        uint256 available = _ensureUsdG(assetsNeeded, block.timestamp, false);
    ...
    function withdrawalReady(bytes32 requestId) external view returns (bool) {
        if (!withdrawalRequests[requestId] || !cycleCommitted || venue.activeTokenId() != 0) return false;
        return availableWithdrawLimit() != 0;
    }

    function withdrawalCycleCommitted() external view returns (bool) {
        return cycleCommitted;
    }
    ...
    function withdrawalCycleChargeableExecutionLoss() external view returns (uint256) {
        return cycleExecutionLoss == 0 ? 0 : cycleFeeBefore;
    }

    function availableWithdrawLimit() public view returns (uint256) {
        if (venue.activeTokenId() != 0) return 0;
        // Steakhouse Vault V2 exposes the official direct `redeem` path but its
        // ERC-4626 `maxRedeem` is not a usable liquidity signal. Readiness is
        // therefore the conservative preview; the actual bounded redeem remains
        // atomic with the Vault claim and fails closed on a liquidity outage.
        uint256 grossLiquid = asset.balanceOf(address(this)) + morphoAdapter.previewAssets();
        (, uint256 pendingFee) = pendingPerformanceFee();
        uint256 liabilities = pendingFee + unremittedFee;
        return grossLiquid > liabilities ? grossLiquid - liabilities : 0;
    }

    function depositsAllowed() external view returns (bool) {
        if (!dependenciesBound() || state != StrategyState.MORPHO_IDLE || cycleCommitted || liveWithdrawalCount != 0) {
            return false;
        }
        // Capital remains USDG/Morpho while idle. At least one healthy future
        // route keeps admission live; the selected market is independently
        // revalidated before and after its eventual opening swap.
        return priceGuard.isHealthy(RobinhoodMarket.ETH) || priceGuard.isHealthy(RobinhoodMarket.NVDA);
    }

    function dependenciesBound() public view returns (bool) {
        return morphoAdapter.controller() == address(this) && venue.controller() == address(this);
    }
    ...
    function estimatedTotalAssetsUpper() external view returns (uint256) {
        (, uint256 pendingFee) = pendingPerformanceFee();
        uint256 gross = estimatedGrossAssetsUpper();
        uint256 liabilities = pendingFee + unremittedFee;
        return gross > liabilities ? gross - liabilities : 0;
    }
    ...
    function maxDeployableAssets() public view returns (uint256) {
        ...
        uint256 claimable = IRobinhoodVaultView(vault).totalClaimableAssets();
        if (idle <= claimable) return 0;
        uint256 shareholderIdle = idle - claimable;
        uint256 reserve = IRobinhoodVaultView(vault).totalAssets() * MIN_VAULT_IDLE_BPS / BPS;
    ...
    function prepareMigration() external onlyVault nonReentrant returns (bool prepared) {
        if (venue.activeTokenId() != 0 || liveWithdrawalCount != 0 || cycleCommitted) return false;
        _crystallizeFee();
        _redeemAllMorpho(block.timestamp, false);
        uint256 transferable = asset.balanceOf(address(this)) - unremittedFee;
        if (transferable != 0) asset.safeTransfer(vault, transferable);
        if (morphoAdapter.shareBalance() != 0 || venue.activeTokenId() != 0) return false;
        accountedAssets = 0;
        return true;
    }
    ...
    function harvest() external onlyRole(KEEPER_ROLE) nonReentrant returns (uint256 profit, uint256 feeAssets) {
        if (state == StrategyState.HALTED) revert InvalidState(StrategyState.MORPHO_IDLE, state);
        if (venue.activeTokenId() != 0) venue.collectFees();
        if (venue.activeTokenId() == 0) return _crystallizeFee();
    ...
    function pendingPerformanceFee() public view returns (uint256 profit, uint256 feeAssets) {
        ...
        return RobinhoodStrategyLib.pendingFee(gross, unremittedFee, accountedAssets, performanceFeeBps);
    }
    ...
    function remitFee() external nonReentrant returns (uint256 remitted) {
        ...
        unremittedFee = 0;
        RobinhoodStrategyLib.remitFee(asset, feeRecipient, remitted);
    ...
    function _redeemAllMorpho(uint256 deadline, bool emergency) internal returns (uint256 assetsReleased) {
        return RobinhoodStrategyLib.redeemAll(address(morphoAdapter), deadline, emergency);
    }

    function _commitCycle() internal {
        cycleCommitted = true;
        cycleCommittedAt = uint64(block.timestamp);
        cycleAssetsBefore = estimatedGrossAssetsExecution();
    ...
    function _crystallizeFee() internal returns (uint256 profit, uint256 feeAssets) {
        ...
        uint256 grossBefore = estimatedGrossAssetsLower();
        uint256 shareholderGrossBefore = grossBefore > unremittedFee ? grossBefore - unremittedFee : 0;
        if (feeAssets != 0) {
            uint256 available = _ensureUsdG(feeAssets, block.timestamp, false);
            if (available < feeAssets) revert WithdrawalNotReady();
            _payFee(feeAssets);
        }
        accountedAssets = shareholderGrossBefore - feeAssets;
        emit PerformanceFeeCharged(profit, feeAssets);
    }

    function _payFee(uint256 amount) internal {
        uint256 paid = RobinhoodStrategyLib.payFee(asset, feeRecipient, amount);
    ...
    function _reduceAccountedAssets(uint256 withdrawn) internal {
        accountedAssets =
            RobinhoodStrategyLib.reducedBasis(accountedAssets, estimatedGrossAssetsLower(), withdrawn, unremittedFee);
    }
