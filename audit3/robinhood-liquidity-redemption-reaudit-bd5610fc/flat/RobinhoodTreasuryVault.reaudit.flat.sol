...
library VaultBDepositLib {
    ...
    function previewInstantRedeem(uint256 shares) external view returns (uint256) {
        ...
        return _instantRedeemNetAssets(vault.convertToAssets(shares), vault.instantRedeemFeeBps());
    }
    ...
    function previewInstantWithdraw(uint256 assets, uint256 virtualShares) external view returns (uint256) {
        ...
        return
            Math.mulDiv(grossAssets, vault.totalSupply() + virtualShares, vault.totalAssets() + 1, Math.Rounding.Ceil);
    }
    ...
    function maxLiquidInstantRedeem(address owner) external view returns (uint256 shares, uint256 assets) {
        ...
        if (feeBps != 0) {
            ...
            if (supply <= minimumResidualShares) return (0, 0);
            uint256 residualCap = supply - minimumResidualShares;
            if (shares > residualCap) shares = residualCap;
        }

        uint256 liquid = vault.availableImmediateLiquidity();
        if (liquid == 0 || shares == 0) return (0, 0);
        assets = _instantRedeemNetAssets(vault.convertToAssets(shares), feeBps);
        ...
        uint256 liquidShares = vault.convertToShares(grossLiquid);
    ...
    function redeemEpochBounds() external view returns (uint256 cutoff, uint256 notBefore) {
        ...
        cutoff = (timestamp / enrollmentWindow + 1) * enrollmentWindow;
        notBefore = cutoff + minimumDelay;
    }
        ...
        returns (uint256 threshold, bool externallyCommitted)
    {
        ...
        threshold = vault.minimumDelayedRedeemDelay() == 0 ? Math.ceilDiv(vault.totalSupply(), 20) : minimumShares;
    ...
    function inspectRedeemRequest(
        ...
        if (requestCount != 0) {
            ...
            uint256 cycleRate = vault.redeemCycleMinAssetsPerShareRay();
            uint16 cycleLoss = vault.redeemCycleMaxLossBps();
            if (!explicitTolerance && (cycleRate != 0 || cycleLoss != maximumLoss)) {
                revert RedeemToleranceBucketMismatch(cycleRate, cycleLoss, minRate, maxLossBps);
            }
            // An existing epoch may be stricter than a joining request, never
            // weaker. A later seat therefore cannot tighten and grief the cohort.
            if (minRate > cycleRate || maxLossBps < cycleLoss) {
                revert RedeemToleranceBucketMismatch(cycleRate, cycleLoss, minRate, maxLossBps);
            }
        }
        ...
        if (existingPlusOne == 0) {
            ...
            uint256 divisor = maximumSeats * (vault.minimumDelayedRedeemDelay() == 0 ? 20 : 5);
            uint256 minimumShares = vault.MIN_REDEEM_SHARES();
            if (divisor != 0) {
    ...
    function cancelExpiredWithdrawal(uint256 requestId, uint256 timeout, bool localCommitted)
        external
...
library VaultBRedemptionLib {
    ...
    function completeRedeemRequest(
        ...
        state.redeemTolerances[requestId] = RedeemToleranceData({minAssets: uint240(minAssets), maxLossBps: maxLossBps});
        state.outstandingRedeemShares += shares;
        ...
        returns (ClaimResult memory result)
    {
        ...
        if (result.committed) {
            ...
            state.redeemCyclePayoutClaimed += result.assets;
        } else {
        ...
        uint256 minimumAssets = state.redeemTolerances[requestId].minAssets;
        if (!result.forceCanceled && result.assets < minimumAssets) {
            revert RedeemCycleToleranceNotMet(result.assets, minimumAssets);
        }
    ...
    function finalizeRedeemClaim(RedeemState storage state, uint256 requestId, ClaimResult calldata result) external {
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
        ...
        if (result.forceCanceled) {
            emit RedeemCanceled(requestId, result.owner, result.shares);
        } else {
    ...
    function recordDeferredRedeemHandle(RedeemState storage state, uint256 requestId, bytes32 deferredId) external {
        _recordDeferredRedeemHandle(state, requestId, deferredId);
    }
    ...
    function finalizeProportionalWithdrawal(
        RedeemState storage state,
        ...
        if (vault.redeemCycleCommittedShares() == vault.redeemCycleSupplySnapshot()) {
            uint256 balance = IERC20(vault.asset()).balanceOf(address(this));
            uint256 required = state.totalClaimableAssets + payoutAssets;
            if (balance > required) {
                uint256 surplus = balance - required;
                address recipient = vault.treasury();
                state.claimableAssets[recipient] += surplus;
                state.totalClaimableAssets += surplus;
                emit RedeemFullSupplySurplusReserved(recipient, surplus);
            }
        }
        ...
        returns (uint256)
    {
        ...
        uint256 payoutAssets = vault.redeemCyclePayoutAssets();
        if (vault.redeemCycleForceSettled()) {
            return Math.mulDiv(payoutAssets, requestShares, vault.redeemCycleCommittedShares());
        }
        IVaultBAsyncStrategy strategy = IVaultBAsyncStrategy(vault.strategy());
        ...
        if (!vault.redeemCycleSettlementInitialized()) {
            (bool valid, uint256 previewPayout) = _previewRedeemCycleSettlement(
                ...
                vault.redeemCycleMaxLossBps()
            );
            if (!valid || previewPayout < _redeemToleranceRequired(vault)) return 0;
    ...
    function _initializeRedeemCycleSettlement(RedeemState storage state) private {
        (uint256 payout, uint256 measured, uint256 charged) = _quoteRedeemCycleSettlement();
    ...
    function _quoteRedeemCycleSettlement() private view returns (uint256 payout, uint256 measured, uint256 charged) {
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
        IVaultBAsyncStrategy strategy = IVaultBAsyncStrategy(vault.strategy());
        uint256 currentAssets = VaultBDepositLib.totalAssetsLowerStrict();
        ...
        uint256 required = _redeemToleranceRequired(vault);
        if (payout < required) revert RedeemCycleToleranceNotMet(payout, required);
    }
    ...
    function _previewRedeemCycleSettlement(
        ...
        uint256 charge = Math.mulDiv(charged, supply - batchShares, supply, Math.Rounding.Ceil);
        if (charge >= batchSnapshot) return (false, 0);
        return (true, batchSnapshot - charge);
    }

    function _redeemToleranceRequired(IVaultBRedeemState vault) private view returns (uint256 required) {
        uint256 committedShares = vault.redeemCycleCommittedShares();
        uint256 minByAssets = Math.mulDiv(
            committedShares, vault.redeemCycleMinAssetsPerShareRay(), REDEEM_RATE_SCALE, Math.Rounding.Ceil
        );
        uint256 referenceAssets =
            Math.mulDiv(vault.redeemCycleAssetsSnapshot(), committedShares, vault.redeemCycleSupplySnapshot());
        uint16 maxLossBps = vault.redeemCycleMaxLossBps();
        uint256 minByLoss = Math.mulDiv(referenceAssets, 10_000 - maxLossBps, 10_000, Math.Rounding.Ceil);
        required = minByAssets > minByLoss ? minByAssets : minByLoss;
    }
}
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function delayedRedeemEnrollmentWindow() public view virtual returns (uint256) {
        return minimumDelayedRedeemDelay();
    }
    ...
    function commitThresholdShares() public view virtual returns (uint256 threshold) {
        ...
        if (_requiresPreSettlement()) return MIN_REDEEM_SHARES;
    ...
    function claimRedeem(uint256 requestId) external nonReentrant returns (uint256 assets) {
        ...
        if (result.forceCanceled) {
            _transfer(address(this), result.owner, result.shares);
        } else {
            _burn(address(this), result.shares);
        }
        VaultBRedemptionLib.finalizeRedeemClaim(_redemptionState(), requestId, result);
    ...
    function forceCancelExpiredRedeem(uint256 requestId) external nonReentrant {
        ...
        if (request.status != RedeemStatus.PENDING) revert RedeemRequestUnknown();
        bool deferred =
            VaultBDepositLib.cancelExpiredWithdrawal(requestId, REDEEM_REQUEST_TIMEOUT, _redeemCycleCommitted);
    ...
    function _recordDeferredRedeemHandle(uint256 requestId, bytes32 deferredId) internal {
        VaultBRedemptionLib.recordDeferredRedeemHandle(_redemptionState(), requestId, deferredId);
    }
    ...
    function claimableRedeemRequest(uint256 requestId) external view returns (uint256 assets) {
        ...
        return
            VaultBRedemptionLib.claimableRedeemRequest(request.strategyRequestId, request.shares, _redeemCycleCommitted);
    }
...
/// requested pro-rata liquidity, so they do not need a vault-wide 20% unwind
/// threshold.
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    /// @notice Deprecated ABI telemetry retained for integrations created before
    /// proportional settlement replaced the terminal 20% batch policy.
    ...
    function availableImmediateLiquidity() public view override returns (uint256) {
        return strategy.availableWithdrawLimit();
    }
    ...
    function settleRedeemCycle() external whenNotPaused nonReentrant {
        ...
        VaultBRedemptionLib.finalizeProportionalWithdrawal(
            _redemptionState(), morphoReleased, lpRecovered, reservedToVault
        );
    }
