...
**Packaging note:** the previous package's libraries flat mistakenly inlined
`RobinhoodVenueLib`, which belongs to the Venue scope. It has been removed; this flat's
contents now match the composition used in every earlier libraries submission.


Answers to three reports on `robinhood-audit11-composite-2e5ccc1a`
...
9. **PUSH0/TSTORE/MCOPY execute on chain 4663** — verified by `eth_call` against the live
   RPC, not assumed.
