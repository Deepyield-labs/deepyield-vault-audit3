...
contract RobinhoodPriceGuard {
    ...
    function emergencyExitPrices(RobinhoodMarket market) public view returns (Prices memory p) {
        ...
        try this.twapTick(market) returns (int24 twapTick_) {
            ...
            if (twapPrice < p.twapUsdGPerRisk) {
    ...
    function upperRiskValue(RobinhoodMarket market, uint256 riskAmount) public view returns (uint256) {
        ...
        try this.healthyPrices(market) returns (Prices memory healthy) {
            ...
            // NAV input. Use the emergency reference, which can only tighten
            // from pinned spot through live TWAP/oracle observations.
            referencePrice = emergencyExitPrices(market).twapUsdGPerRisk;
        }
        return FullMath.mulDiv(riskAmount, referencePrice, ONE_RISK_TOKEN);
    }
...
library RobinhoodVenueLib {
    ...
    function closeRiskPrice(uint256, uint256 twapPrice, bool) external pure returns (uint256) {
        // Normal exit is anchored to TWAP; emergencyExitPrices has already
        // tightened this same field to min(spot, live TWAP, live oracle).
        return twapPrice;
    }
...
library RobinhoodStrategyLib {
    ...
    /// @notice Calls the immutable Venue and classifies only failures for which
    /// the guardian escalation timer may be armed. All unrelated failures keep
    /// their original revert data and the complete operation rolls back.
    function closeVenue(IERC20 asset, address venueAddress, BoundedUniswapV3Venue.CloseParams calldata p)
        external
    ...
    function commitVaultSnapshot(address vault, uint256 grossBefore, uint256 unremitted, uint256 basis, uint16 feeBps)
        external
        returns (bool committed)
    {
        IRobinhoodVaultCommit root = IRobinhoodVaultCommit(vault);
        if (root.outstandingRedeemShares() < root.commitThresholdShares()) return false;
        uint256 feeBefore;
    ...
    function closeLoss(
        ...
        uint16 feeBps,
        uint256 feeBefore
    ) external pure returns (uint256 executionLoss, uint256 chargeable) {
        if (grossBefore <= grossAfter) return (0, 0);
        executionLoss = grossBefore - grossAfter;
        (, uint256 feeAfter) = _pendingFee(grossAfter, unremitted, basis, feeBps);
        uint256 feeOffset = feeBefore > feeAfter ? feeBefore - feeAfter : 0;
        chargeable = executionLoss > feeOffset ? executionLoss - feeOffset : 0;
    }
    ...
    function _ensureUsdGBestEffort(
        ...
        if (shares == 0 || preview == 0) return available;
        ...
        try morpho.redeem(sharesNeeded, minimum, deadline, false) returns (uint256 released) {
    ...
    function emergencyRedeemToVault(
        IERC20 asset,
        address morphoAddress,
        address vault,
        uint256 suppliedFloor,
        uint256 deadline
    ) external returns (uint256 shares, uint256 released) {
        (shares, released) = _redeemAll(morphoAddress, deadline, true, suppliedFloor);
        if (released != 0) asset.safeTransfer(vault, released);
    }
        ...
        returns (uint256)
    {
        ...
        return asset.balanceOf(address(this)) + IRobinhoodMorphoCapital(morphoAddress).previewAssets() + venueAssets;
    }
    ...
    function _recoverableCloseFailure(bytes memory reason) private pure returns (bool) {
        ...
        return selector == BoundedUniswapV3Venue.CloseExecutionInfeasible.selector
            || selector == RobinhoodPriceGuard.PriceDivergence.selector
...
contract RobinhoodTreasuryStrategy is IVaultBAsyncStrategy, AccessControlDefaultAdminRules, ReentrancyGuard {
        ...
        returns (uint256 assetsRecovered)
    {
        ...
        bytes32 jobId = activeJobId;
        if (!exiting) {
            // The cycle snapshot remains immutable, but the execution-loss
            // baseline starts immediately before this close. Legitimate liquid
            // payouts made from Morpho after commit must not be misclassified
            // as LP execution loss.
            _exitGrossBefore = estimatedGrossAssetsLower();
            _exitReturnsToHalted = halted || p.emergency;
            state = StrategyState.EXITING;
        }
        RobinhoodStrategyLib.CloseResult memory closeResult = RobinhoodStrategyLib.closeVenue(asset, address(venue), p);
        ...
        if (!closeResult.terminal) {
            if (!p.emergency) _recordNormalCloseFailure(bytes4(0));
        ...
        if (liveWithdrawalCount != 0 && !cycleCommitted) {
            uint256 grossBefore = _exitGrossBefore;
            if (RobinhoodStrategyLib.commitVaultSnapshot(
                    vault, grossBefore, unremittedFee, accountedAssets, performanceFeeBps
                )) {
        ...
        if (cycleCommitted) {
            (cycleExecutionLoss, cycleFeeBefore) = RobinhoodStrategyLib.closeLoss(
                _exitGrossBefore, grossAfter, unremittedFee, accountedAssets, performanceFeeBps, cycleFeeBefore
            );
        }
        ...
        returns (uint256 assetsReleased)
    {
        ...
        (, assetsReleased) =
            RobinhoodStrategyLib.emergencyRedeemToVault(asset, address(morphoAdapter), vault, minAssetsOut, deadline);
        if (assetsReleased != 0) _reduceAccountedAssets(assetsReleased);
        emit StrategyHalted(msg.sender);
    }
    ...
    function withdrawToVault(uint256 assetsNeeded) external onlyVault nonReentrant returns (uint256 withdrawn) {
        ...
        _crystallizeFee();
        withdrawn = RobinhoodStrategyLib.withdrawBestEffortToVault(
            asset, address(morphoAdapter), vault, assetsNeeded, unremittedFee, block.timestamp
        );
    ...
    function managerWithdrawAll() external onlyRole(KEEPER_ROLE) nonReentrant returns (uint256 withdrawn) {
        if (liveWithdrawalCount != 0) return 0;
        _crystallizeFee();
        withdrawn = RobinhoodStrategyLib.redeemAllAndTransfer(
            asset, address(morphoAdapter), vault, unremittedFee, block.timestamp, state == StrategyState.HALTED
        );
        if (venue.activeTokenId() == 0) accountedAssets = 0;
        else if (withdrawn != 0) _reduceAccountedAssets(withdrawn);
    }
    ...
    function requestWithdrawal(bytes32 requestId, uint256) external onlyVault nonReentrant {
        ...
        if (liveWithdrawalCount == 0 && state == StrategyState.EXITING && !cycleCommitted) {
            _exitGrossBefore = estimatedGrossAssetsLower();
        }
        ...
        returns (uint256 withdrawn)
    {
        ...
        if (!cycleCommitted && venue.activeTokenId() != 0) revert WithdrawalNotReady();
        _crystallizeFee();
    ...
    function estimatedGrossAssetsExecution() public view returns (uint256 gross) {
        // Execution and redemption both use the same conservative lower Venue
        // geometry; retain the public compatibility view without a second path.
        return estimatedGrossAssetsLower();
    }
    ...
    function prepareMigration() external onlyVault nonReentrant returns (bool prepared) {
        ...
        _crystallizeFee();
        RobinhoodStrategyLib.redeemAll(address(morphoAdapter), block.timestamp, false);
    ...
    function harvest() external onlyRole(KEEPER_ROLE) nonReentrant returns (uint256 profit, uint256 feeAssets) {
        if (state == StrategyState.HALTED) revert InvalidState(StrategyState.MORPHO_IDLE, state);
        if (venue.activeTokenId() != 0 && !venue.activePositionBurned()) venue.collectFees();
    ...
    function _recordNormalCloseFailure(bytes4 selector) internal {
        if (normalCloseFailedAt != 0) return;
