...
library RobinhoodVenueLib {
    ...
    function recordExecutionLoss(
        ...
        // The budget denominator is the PEAK exposure seen in the window, not
        // cumulative admitted flow. Recycling the same capital through repeated
        // open/close cycles therefore cannot buy a fresh allowance each time,
        // and an emergency exit (reference 0 by design) cannot pin a zero
        // denominator: the mandatory follow-up close, and any later admission,
        // raise the base to the exposure actually at risk. An admission raises
        // it only AFTER the inherited-loss check below, so a larger next open
        // cannot buy its way past an already exhausted budget.
        ...
        if (!enforce) {
            if (referenceValue > window.base) window.base = referenceValue;
        ...
        )
    {
        ...
        // Ceiling rounding can reach the full liquidity for a legitimate
        // part < whole (a near-total cohort on a small position). That is a
        // valid partial exit, not an invalid amount: leave one unit behind so
        // the position stays live instead of reverting the cohort (job 848 F-1).
        if (proportional >= p.liquidity) proportional = uint256(p.liquidity) - 1;
        if (proportional == 0 || proportional > type(uint128).max) revert InvalidAmount();
...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    /// @notice Value-axis counterpart of MAX_ZERO_PREVIEW_DUST_SHARES. A preview
    /// worth no more than this fraction of cost basis is materially zero. An
    /// exact-equality test on an externally controlled value is not a workable
    /// definition of "worthless": one wei of accrual, or a one-wei donation into
    /// the vault, would both block the write-off from ever arming and discard an
    /// already-armed window. A relative floor is grief-proof in both directions.
    uint16 public constant ZERO_PREVIEW_DUST_BPS = 10;
    ...
    event AssetSwept(uint256 assets);
    ...
    function park(
        ...
        // Read what the bundle actually consumed. Reading after a reset could
        // only ever observe our own write, so the check would never fire and an
        // under-consuming bundle would still credit the full requested amount
        // to the cost basis.
        //
        // No reset follows: a nonzero allowance reverts the whole call, and a
        // zero one needs no clearing. The unconditional forceApprove(0) that
        // used to sit here was therefore dead, and it was the one call in this
        // function that a token refusing zero-value approvals could brick.
    ...
    /// @notice Returns raw USDG sitting on the adapter to the controller. The
    /// adapter is never meant to hold the asset: `park` sweeps it as part of a
    /// deposit, but that sweep is gated on the external fee policy, so a
    /// donation or an operational mistake would otherwise strand the balance
    /// for as long as the curator keeps a nonzero fee. This path is deliberately
    /// free of that gate. It can only pay the immutable controller, which
    /// forwards spendable USDG to the immutable Vault, so it adds no new
    /// destination for funds.
    function sweepAsset() external onlyController nonReentrant returns (uint256 swept) {
        swept = asset.balanceOf(address(this));
        if (swept == 0) revert InvalidAmount();
        asset.safeTransfer(controller, swept);
        emit AssetSwept(swept);
    }
        ...
        returns (bytes32 exitId, uint256 shares, uint64 readyAt, uint64 expiresAt)
    {
        ...
        // Arming requires the ABSENCE of a proven material value: a preview that
        // is materially zero, or one that cannot be read at all. This knob has
        // swung through every position across four rounds, and each extreme
        // failed on the other side:
        //   - "zero or unreadable" alone let a solvent-but-unreadable position
        //     burn against no floor (round-12 High);
        //   - "proven zero" alone deadlocked an unreadable position worth less
        //     than the ordinary path's half-of-basis floor — the ordinary path
        //     demands >= 50%, the write-off demands a proven zero, and an
        //     unreadable preview proves neither (round-15 High).
        // The unreadable case is therefore admitted again, but under every
        // protection added since round 11: two observations separated by the
        // 24h delay, a 6h execution window, controller-only cancel, and a latch
        // that dies the moment any observation shows material value.
        (bool armPriced, uint256 preview) = _tryPreviewRedeem(shares);
        if (armPriced && !_materiallyZero(preview)) revert ZeroPreviewExitRecovered(preview);
        ...
        returns (uint256 sharesBurned, uint256 assetsReceived)
    {
        ...
        // The second independent observation must still find no proven material
        // value — materially zero, or still unreadable.
        (bool execPriced, uint256 preview) = _tryPreviewRedeem(sharesBurned);
        if (execPriced && !_materiallyZero(preview)) revert ZeroPreviewExitRecovered(preview);
        ...
        returns (uint256 assetsReceived)
    {
        ...
        if (block.timestamp > deadline) revert DeadlineExpired(deadline);
        // One choke point for every non-emergency egress: a materially impaired
        // sleeve is realized only through the deliberate guardian paths (859 H-2).
        if (!emergency && !zeroPreviewAuthorized && materialImpairment()) revert MaterialImpairment();
        ...
        if (!priced) {
            // The half-of-basis floor governs the UNAUTHORIZED unreadable exit:
            // a stated floor of one wei is not a valuation, so it is anchored to
            // the same line materialImpairment() already draws.
            //
            // The authorized write-off is exempt from both checks in this
            // branch, not from one of them. It can arm on a persistently
            // unreadable preview (see armZeroPreviewEmergencyExit for the four
            // rounds of history), so it legitimately lands here with a zero
            // floor; gating it on either check re-creates the arm-but-never-
            // execute deadlock twice fixed already. Its protection is the
            // discipline it arrived with: two delayed observations, the 6h
            // window, and a latch that dies on any material valuation.
            if (!zeroPreviewAuthorized) {
        ...
        // An unauthorized caller may state a zero floor only for the exact
        // zero/zero case: a sub-asset-unit residual that legitimately previews
        // to zero. The exemption is position-level, so a zero-preview position
        // cannot be drained around the delayed write-off in dust-sized chunks.
        //
        // The authorized write-off is exempt from the whole test, not from half
        // of it. It arms and executes on the RELATIVE definition of worthless
        // (_materiallyZero), so a position that legitimately qualifies there
        // still previews nonzero here; leaving `preview != 0` outside the
        // authorization check made the mechanism able to arm but never execute.
        if (
            !zeroPreviewAuthorized && minAssetsOut == 0
                && (preview != 0 || parkedShares > MAX_ZERO_PREVIEW_DUST_SHARES)
        ) {
    ...
    function _executeRedeem(
        ...
        //
        // Consequence worth stating, because it is deliberate: while an
        // untracked donation is outstanding, an ordinary `redeem(parkedShares)`
        // will not drive `parkedShares` to zero — the donation is consumed
        // first. A full exit therefore passes `shareBalance()`, not
        // `parkedShares`. The alternative (attributing the burn to the tracked
        // position first) is the reverse attack: repeated donate-and-redeem
        // cycles would walk the tracked basis to zero and disarm both
        // impairment circuit breakers while the real position stayed intact.
    ...
    function _latchRecovered() private view returns (bool) {
        ...
        // Recovery means the position is worth something again, not that a dust
        // amount appeared: the same floor that governs arming governs release.
        return priced && !_materiallyZero(preview);
    }

    /// @dev True when the preview is worthless relative to what was paid for the
    /// position. With no cost basis recorded only an exact zero qualifies.
    function _materiallyZero(uint256 preview) private view returns (bool) {
        uint256 basis = parkedAssets;
        if (basis == 0) return preview == 0;
        return preview * 10_000 <= basis * ZERO_PREVIEW_DUST_BPS;
    }
...
library VaultBRedemptionLib {
    ...
    function _latchCohortSeal(RedeemState storage state) private {
        ...
        if (threshold != 0 && state.outstandingRedeemShares >= threshold) {
            ...
            // Freeze the bar together with the seal. The threshold tracks
            // min(live supply, supply at queue open), so an instant exit lowers
            // it and a later deposit raises it back. Without this freeze a
            // cohort could latch the seal at a depressed bar and then be unable
            // to clear the restored one — sealed, uncommittable, and unable to
            // cancel out, because the seal is what forbids cancelling.
    ...
    function finalizeRedeemClaim(
        ...
        if (!result.forceCanceled && result.committed && state.outstandingRedeemCount == 0) {
            ...
            uint256 residual = state.redeemCyclePayoutAssets > claimedSoFar
                ? state.redeemCyclePayoutAssets - claimedSoFar
                : 0;
            if (residual != 0) {
                address recipient;
    ...
    function forceCancelExpiredRedeem(
        ...
        if (!deferred && state.outstandingRedeemShares >= vault.commitThresholdShares()) {
            // Same rule as cancelRedeem: a member cannot leave a sealed, committable
            // cohort through the timeout door either (job 866 [1]). A cohort that
            // fell below the bar keeps the timeout exit (855 F-2).
            if (state.redeemCohortSealed && (msg.sender == request.owner || msg.sender == request.receiver)) {
        ...
        returns (uint256 payout, uint256 measured, uint256 charged, uint256 currentAssets)
    {
        ...
        // Live supply, not the commit-time snapshot: the cohort's escrowed
        // shares are still in totalSupply until burned at claim, while instant
        // exits between commit and settlement-init shrink BOTH supply and
        // assets. Dividing live assets by a frozen supply underpaid the cohort
        // by exactly the exited fraction (job 855 F-1).
...
library RobinhoodStrategyLib {
    ...
    function policyWithdrawLimit(
        ...
        (uint256 headroom, uint256 spendableVaultIdle, uint256 strategyLiquid) =
            _policyHeadroom(asset, vault, morphoAddress, venueAddress, unremitted, basis, feeBps, maximumLpBps);
        uint256 strategyHeadroom = headroom > spendableVaultIdle ? headroom - spendableVaultIdle : 0;
        return strategyHeadroom < strategyLiquid ? strategyHeadroom : strategyLiquid;
    }
    ...
    function policyExitCeiling(
        ...
        if (!lpActive) return type(uint256).max;
        // Cap 0 means no LP exposure is tolerated: nothing may leave while the
        // LP is live (consistent with policyWithdrawLimit), not "unbounded".
        if (maximumLpBps == 0) return 0;
        (uint256 headroom,,) =
            _policyHeadroom(asset, vault, morphoAddress, venueAddress, unremitted, basis, feeBps, maximumLpBps);
        return headroom;
    ...
    function _policyHeadroom(
        ...
        uint16 feeBps,
        uint16 maximumLpBps
    ) private view returns (uint256 headroom, uint256 spendableVaultIdle, uint256 strategyLiquid) {
        uint256 rawVaultIdle = asset.balanceOf(vault);
        uint256 claimable = IRobinhoodVaultCommit(vault).totalClaimableAssets();
        spendableVaultIdle = rawVaultIdle > claimable ? rawVaultIdle - claimable : 0;
        (, uint256 morphoAssets) = _tryPreviewAssets(morphoAddress);
        uint256 direct = asset.balanceOf(address(this));
        uint256 lowerGross = direct + _feeBalanceAddback(asset) + morphoAssets
        ...
        uint256 minimumPostExitNav =
            Math.mulDiv(IRobinhoodVenueValue(venueAddress).estimatedValueUpper(), BPS, maximumLpBps, Math.Rounding.Ceil);
        headroom = globalNet > minimumPostExitNav ? globalNet - minimumPostExitNav : 0;
        uint256 grossStrategyLiquid = direct + morphoAssets;
        strategyLiquid = grossStrategyLiquid > liabilities ? grossStrategyLiquid - liabilities : 0;
    }
    ...
    function _redeemMorphoFraction(
        ...
        // Order matters: a caller-supplied bound may only RAISE the protocol's
        // own floor, never stand in for one that could not be derived at all.
        // Testing after the override let any nonzero keeper bound satisfy a
        // check meant to catch an unpriced sleeve.
        if (minimum == 0) {
            // A genuine zero for a sub-dust residual is a priced zero: skip the
            // Morpho leg instead of freezing settlement (852 H-5). Only a
            // material zero preview is an outage.
            if (!morpho.materialZeroPreview()) return (0, 0);
            revert StrategyUnavailable();
        }
    ...
    function crystallizeFee(
        ...
        uint256 paid;
        if (totalOwed != 0) {
            // `protectedBalance` is cash already earmarked for an exiting cohort;
            // fee remittance may not raid it (job 853 #2).
            uint256 available = _ensureUsdGBestEffort(asset, morphoAddress, totalOwed, protectedBalance, block.timestamp);
    ...
    function tryGrossAssets(IERC20 asset, address morphoAddress, address venueAddress, bool upper)
        external
        view
        returns (bool priced, uint256 gross)
    {
        return _tryGrossAssets(
            asset,
            morphoAddress,
            venueAddress,
            upper ? IRobinhoodVenueValue.estimatedValueUpper.selector : IRobinhoodVenueValue.estimatedValueLower.selector
        );
    }
    ...
    function rebaseBasisAfterRelease(
        ...
        // Same mark as reducedBasisAfterWithdrawal: the execution mark, guarded
        // by the tolerant Morpho preview. The haircut lower mark could rebase
        // the fee basis to zero after a burn and charge fees on losses (852 H-3).
        (bool priced,) = _tryPreviewAssets(morphoAddress);
...
library RobinhoodSettlementLib {
    ...
    function emergencyRedeemMorpho(
        ...
        // With a cycle committed the idle is the cohort's settlement reserve:
        // keep it here for settleWithdrawalCycle instead of sweeping it (859 H-6).
        assetsReleased = redemption.cycleCommitted
            ? 0
            : RobinhoodStrategyLib.transferAvailableToVault(c.asset, c.vault, accounting.unremittedFee);
        if (assetsReleased != 0) {
        ...
        returns (uint256 withdrawn)
    {
        ...
        if (depleted && _venueEmpty(c.venue)) {
        ...
        returns (bool prepared)
    {
        ...
        if (accounting.unremittedFee != 0 || !depleted || !_venueEmpty(c.venue)) return false;
        accounting.accountedAssets = 0;
        return true;
    }
    ...
    function requireLiquidityPolicy(
        ...
        // An unavailable mark cannot be turned into a policy verdict — but it
        // cannot license one either. With no live position the ceiling is
        // trivially satisfied and an outage must not block payments. With a
        // live position the exposure exists and simply cannot be measured, and
        // every caller here has already moved capital by this point, so the
        // only honest answer is to refuse: close the position first, then move
        // capital. The emergency-close path remains available for exactly that.
        // A burned position still holds priced risk inventory: it is exposure (859 H-1).
        bool positionLive = IRobinhoodSettlementVenue(venueAddress).activeTokenId() != 0
            || IRobinhoodSettlementVenue(venueAddress).activePositionBurned();
        if (!lowerPriced || !upperPriced) {
            if (positionLive) revert RobinhoodStrategyLib.StrategyUnavailable();
            return;
        }
        ...
        // Same rule for the sleeve: an outage is not a zero balance, and with
        // LP exposure live it cannot be waved through either.
        ...
        uint256 rawVaultIdle = asset.balanceOf(vault);
        uint256 claimable = IRobinhoodVaultCommit(vault).totalClaimableAssets();
        // Cash this very call just moved to the Vault is not reserve (859 H-5).
        uint256 spendableVaultIdle = rawVaultIdle > claimable ? rawVaultIdle - claimable : 0;
        spendableVaultIdle = spendableVaultIdle > excludedVaultIdle ? spendableVaultIdle - excludedVaultIdle : 0;
        uint256 grossLiquid = asset.balanceOf(address(this)) + morphoAssets + spendableVaultIdle;
        uint256 grossLower = lpLower + grossLiquid;
        // The fee basis covers Strategy capital only; Vault idle must not be
        // charged a phantom performance fee (job 853 #4).
        uint256 strategyGross = grossLower - spendableVaultIdle;
        (, uint256 pending) = RobinhoodStrategyLib.pendingFee(strategyGross, unremitted, basis, feeBps);
        ...
        returns (uint256)
    {
        ...
        return RobinhoodStrategyLib.policyWithdrawLimit(
            ...
            _self().activeTokenId() != 0 || IRobinhoodSettlementVenue(c.venue).activePositionBurned()
        );
    }

    function availableExitCeiling(AccountingStorage storage accounting, RedemptionStorage storage redemption)
        external
        view
        returns (uint256)
    {
        Config memory c = _config();
        return RobinhoodStrategyLib.policyExitCeiling(
            c.asset,
            c.vault,
            c.morpho,
            c.venue,
    ...
    function settleWithdrawalCycle(
        ...
        // One tolerant read, not a check followed by a reverting read: the
        // previous shape asked tryPreviewAssets and then called grossAssets,
        // which reverts on exactly the condition just tested — so the guard
        // below was dead and a routine adapter outage froze settlement.
        (bool grossPriced, uint256 lowerGross) =
            RobinhoodStrategyLib.tryGrossAssets(c.asset, c.morpho, c.venue, false);
        ...
        (accounting.accountedAssets, accounting.unremittedFee) = _checkpointFeeProtected(
            c.asset,
            c.morpho,
            c.venue,
            c.feeRecipient,
            ...
            accounting.accountedAssets,
            redemption.performanceFeeBps,
            realized.morphoReleased + realized.lpRecovered
        );
        realized.reservedToVault = RobinhoodStrategyLib.transferCycleReserve(
            ...
            // The capped floor settleCycleAssets already derived (859 H-4).
            realized.protectedBalance
        );
        ...
        // Re-check the ratio whenever the Venue still carries exposure (live or
        // burned); an absolute-mark comparison never fired when the LP leg was
        // skipped or the mark was unavailable (859 H-3).
        if (
            IRobinhoodSettlementVenue(c.venue).activeTokenId() != 0
                || IRobinhoodSettlementVenue(c.venue).activePositionBurned()
        ) {
            requireLiquidityPolicy(
                c.asset,
                c.vault,
    ...
    function finalizeWithdrawalCycleReserve(
        ...
        // The underwater reduction is staged in full by settleWithdrawalCycle as
        // the cohort's pro-rata slice of basis, so nothing is added here.
        //
        // Three earlier rounds of this engagement each tried to add the
        // reserve surplus (reserved minus paid) on top, with progressively
        // tighter bounds. All of them were double reductions: the pro-rata slice
        // is already complete, and reserved cash that was never paid out has not
        // left the system — it sits in Vault idle owned by the holders who
        // remain, so it cannot reduce their fee high-water mark at all. Adding
        // it charged those holders performance fee on their own principal.
        uint256 basisReduction = redemption.cycleBasisReduction;
        // The staged figure is absolute and was computed against the basis as it
        // stood at settlement. An emergency release between settlement and here
        // rebases that basis downward, and subtracting the stale absolute figure
        // from the reduced basis removes the cohort's share twice — the second
        // time out of the remaining holders' high-water mark, which their next
        // recovery then pays performance fee on.
        //
        // Clamping to the cohort's pro-rata share of the CURRENT basis keeps the
        // emergency paths callable mid-cycle (blocking them would disable the
        // emergency exactly when it is needed) while making a double reduction
        // arithmetically impossible.
        IRobinhoodVaultCommit root = IRobinhoodVaultCommit(_config().vault);
        uint256 supplySnapshot = root.redeemCycleSupplySnapshot();
        if (supplySnapshot != 0) {
            uint256 proRataNow =
                Math.mulDiv(accounting.accountedAssets, root.redeemCycleCommittedShares(), supplySnapshot);
            if (basisReduction > proRataNow) basisReduction = proRataNow;
        }
        accounting.accountedAssets =
    ...
    function cancelWithdrawal(
        ...
        if (redemption.liveWithdrawalCount == 0) {
            if (redemption.cycleLiquidityPrepared && !redemption.cyclePayoutFinalized) {
                ...
                if (staged != 0) {
                    // Same clamp finalizeWithdrawalCycleReserve applies, for the
                    // same reason: the staged figure is absolute and was computed
                    // against the basis at settlement, so an emergency release in
                    // between would otherwise let it be taken twice — the second
                    // time out of the remaining holders' high-water mark.
                    IRobinhoodVaultCommit cancelRoot = IRobinhoodVaultCommit(_config().vault);
                    uint256 cancelSupply = cancelRoot.redeemCycleSupplySnapshot();
                    if (cancelSupply != 0) {
                        uint256 cancelProRata = Math.mulDiv(
                            accounting.accountedAssets, cancelRoot.redeemCycleCommittedShares(), cancelSupply
                        );
                        if (staged > cancelProRata) staged = cancelProRata;
                    }
                    accounting.accountedAssets =
    ...
    function commitCycle(
        ...
        // And a live request is not by itself a commitment. The Vault owns that
        // decision; latching ahead of it lets one dust-sized request close every
        // fee, capital and migration path until the requester cancels, with no
        // override anywhere. Read the authoritative flag instead of trusting the
        // caller to have honoured the snapshot's verdict.
        if (!IRobinhoodVaultCommit(_config().vault).redeemCycleCommitted()) revert WithdrawalNotReady();
    ...
    function _checkpointFee(
        ...
        (nextBasis, nextUnremitted,,) =
            _checkpointFeeWithAmounts(asset, morpho, venue, feeRecipient, oldUnremitted, oldBasis, feeBps, 0);
    }

    /// @dev Checkpoint that keeps `protectedBalance` (an exiting cohort's realized
    /// cash) out of reach of fee remittance.
    function _checkpointFeeProtected(
        IERC20 asset,
        address morpho,
        address venue,
        address feeRecipient,
        ...
        (nextBasis, nextUnremitted,,) = _checkpointFeeWithAmounts(
            asset, morpho, venue, feeRecipient, oldUnremitted, oldBasis, feeBps, protectedBalance
        );
    }
    ...
    function _checkpointFeeWithAmounts(
        ...
        bool priced;
        (priced, profit, feeAssets, nextBasis, nextUnremitted) = RobinhoodStrategyLib.crystallizeFee(
            asset, morpho, venue, feeRecipient, oldUnremitted, oldBasis, feeBps, protectedBalance
        );
    ...
    function _venueEmpty(address venueAddress) private view returns (bool) {
        return IRobinhoodSettlementVenue(venueAddress).activeTokenId() == 0
            && !IRobinhoodSettlementVenue(venueAddress).activePositionBurned();
    }
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function forceSettleStuckCycle() external nonReentrant {
        if (!_redeemCycleCommitted) {
            ...
            // The threshold gate governs VOLUNTARY commits only. This branch
            // adopts a commitment the strategy has already made and that an
            // external witness confirms, so gating it on the vault's own queue
            // size makes the recovery path unable to fire in exactly the state
            // it exists for: a sub-threshold queue whose strategy already
            // committed. The witness below remains the authorization.
...
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    ...
    function commitThresholdShares() public view override returns (uint256 threshold) {
        if (outstandingRedeemCount == 0) return 0;
        // The seal is sticky, so the bar it latched against has to be sticky
        // too. This override replaces the base implementation wholesale, so it
        // must repeat the freeze: without it the product recomputes a live bar,
        // and a cohort sealed at a depressed one can neither clear the restored
        // bar nor cancel out of the seal that forbids cancelling.
        uint256 sealedThreshold = _redemptionState().redeemCycleSealThreshold;
        if (sealedThreshold != 0) return sealedThreshold;
