...
// NOTE: RobinhoodStrategyLib function bodies (71 functions) are replaced by reverting
// stubs here. The library is reviewed in full in RobinhoodStrategyLibraries.reaudit.flat.sol. Every function of
...
library RobinhoodStrategyLib {
    ...
    function crystallizeFee( IERC20 asset, address morphoAddress, address venueAddress, address recipient, uint256 unremitted, uint256 basis, uint16 feeBps ) external returns (bool priced, uint256 profit, uint256 feeAssets, uint256 nextBasis, uint256 nextUnremitted) {
        revert();
    }
...
// NOTE: RobinhoodSettlementLib function bodies (33 functions) are replaced by reverting
// stubs here. The library is reviewed in full in RobinhoodStrategyLibraries.reaudit.flat.sol. Every function of
// it is external, so each call from the contract above is a delegatecall to
// the linked address: the caller's deployed runtime is byte-identical with or
// without these bodies, which this package's ABI/runtime equivalence check
...
library RobinhoodSettlementLib {
    ...
    function requireLiquidityPolicy( IERC20 asset, address vault, address morphoAddress, address venueAddress, uint256 unremitted, uint256 basis, uint16 feeBps, uint16 minimumLiquidBps, uint16 maximumLpBps ) public view {
        revert();
    }
    ...
    function _checkpointFeeWithAmounts( IERC20 asset, address morpho, address venue, address feeRecipient, uint256 oldUnremitted, uint256 oldBasis, uint16 feeBps ) private returns (uint256 nextBasis, uint256 nextUnremitted, uint256 profit, uint256 feeAssets) {
        revert();
    }
