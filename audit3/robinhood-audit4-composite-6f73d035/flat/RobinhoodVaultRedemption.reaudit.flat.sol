...
library VaultBDepositLib {
    ...
    /// @notice Anchor observation taken after every deposit, mint, withdraw and
    /// redeem. The strict lower NAV is admitted only inside the current band, so
    /// one transaction can move the anchor by at most the elapsed drift; delayed
    /// settlement still records the exact settled value.
    function refreshedInstantNavReference() external view returns (uint256 assets, uint256 supply) {
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
        supply = vault.totalSupply();
        if (supply == 0) return (0, 0);
        assets = _totalAssetsLowerStrict(
            IERC20(vault.asset()), address(this), IVaultBAsyncStrategy(vault.strategy()), vault.totalClaimableAssets()
        );
        (uint256 floor, uint256 cap) = _navReferenceBand(vault);
    ...
    /// @notice Re-arm the active strategy's pull allowance. An emergency
    /// (paused) migration deliberately installs the candidate with a zero
    /// allowance; once the admin unpauses, ordinary deployment must work again.
    function armStrategyAllowance() external {
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
...
library VaultBRedemptionLib {
    ...
    function prepareUnpause(RedeemState storage state) external {
        ...
        VaultBDepositLib.armStrategyAllowance();
    }
    ...
    function refreshInstantNavReference(RedeemState storage state) external {
        (uint256 assets, uint256 supply) = VaultBDepositLib.refreshedInstantNavReference();
        ...
        returns (address owner, uint256 shares)
    {
        ...
        if (state.outstandingRedeemShares >= vault.commitThresholdShares()) revert RedeemCohortSealed();
    ...
    function _settlementCohortBasis(
        ...
        return Math.mulDiv(settlementAssets, committedShares, supplySnapshot) + chargeableLoss;
    }
...
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    ...
    /// sealed cohort always has a buffer before it becomes committable and no
    /// late seat change can coincide with the commit boundary.
    function delayedRedeemEnrollmentWindow() public pure override returns (uint256) {
        return ROBINHOOD_DELAYED_REDEEM_DELAY / 2;
    }
