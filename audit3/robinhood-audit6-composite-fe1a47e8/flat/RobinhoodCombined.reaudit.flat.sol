...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    function park(
        ...
        // The adapter never holds USDG: return execution dust and any prior
        // direct donation to the sole allowed recipient.
    ...
    {
        bytes32 pendingExitId = zeroPreviewEmergencyExitId;
        if (pendingExitId != bytes32(0) && morphoVault.previewRedeem(zeroPreviewExitShares) == 0) {
            revert ZeroPreviewExitPending(pendingExitId);
        }
        ...
        returns (uint256 assetsReceived)
    {
        ...
        if (
            minAssetsOut == 0
                && (preview != 0 || (!zeroPreviewAuthorized && sharesBefore > MAX_ZERO_PREVIEW_DUST_SHARES))
        ) {
            revert InvalidAmount();
        }
        ...
        parkedAssets = sharesBefore == 0 ? 0 : Math.mulDiv(parkedAssets, sharesBefore - shares, sharesBefore);
        ...
        // The emitted flag reflects the authorized write-off path, not a
        // caller-supplied label.
        emit Redeemed(shares, assetsReceived, zeroPreviewAuthorized);
    }
...
library RobinhoodStrategyLib {
    ...
    function settleCycleAssets(
        ...
        uint256 protectedBalance = unremitted + pendingAfter - feeShare;
