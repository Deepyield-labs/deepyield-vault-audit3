...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
        ...
        returns (bytes32 exitId, uint256 shares, uint64 readyAt, uint64 expiresAt)
    {
        ...
        // Arming requires the absence of a proven value: an exact zero, or a
        // preview that cannot be read at all. Admitting the unreadable case is
        // what keeps custody from locking when the vault stops pricing, and the
        // second independent observation after the delay still has to agree.
        ...
        returns (uint256 sharesBurned, uint256 assetsReceived)
    {
        ...
        // The second independent observation must still find no proven value.
        ...
        returns (uint256 assetsReceived)
    {
        ...
        if (!priced) {
            if (minAssetsOut == 0 && !zeroPreviewAuthorized) revert InvalidAmount();
            if (!zeroPreviewAuthorized) {
                // A stated floor of one wei is not a valuation either. Anchor it
                // to the same half-of-basis line materialImpairment() already
                // draws, so a material position cannot be burned for nothing
                // merely because the preview is unreadable. Custody still never
                // locks: an unpriced position can arm the delayed write-off,
                // which carries this floor's authorization instead.
    ...
    function _executeRedeem(
        ...
        uint256 ownBurned = shares < ownBefore ? shares : ownBefore;
