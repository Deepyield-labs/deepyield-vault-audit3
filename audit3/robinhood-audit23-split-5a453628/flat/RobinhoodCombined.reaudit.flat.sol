...
library RobinhoodStrategyLib {
    ...
    function _redeemMorphoFraction(
        ...
        // A materially impaired sleeve is realized only through the deliberate
        // guardian paths (emergency redeem / write-off), never silently inside
        // a settlement at a market floor (852 H-4).
        if (morpho.materialImpairment()) revert StrategyUnavailable();
    ...
    function _ensureUsdGBestEffort(
        ...
        if (IRobinhoodMorphoCapital(morphoAddress).materialImpairment()) return available;
        ...
        returns (uint256 shares, uint256 released)
    {
        ...
        if (!emergency && shares != 0 && morpho.materialImpairment()) revert StrategyUnavailable();
        if (shares == 0) return (0, 0);
        uint256 preview = morpho.previewRedeem(shares);
...
library RobinhoodSettlementLib {
    ...
    function requireLiquidityPolicy(
        ...
        uint256 strategyGross = grossLower - (rawVaultIdle > claimable ? rawVaultIdle - claimable : 0);
    ...
    function settleWithdrawalCycle(
        ...
        // Sampled BEFORE the unwind, or the post-settlement policy re-check below
        // compares the mark with itself and never fires (job 853 #1).
