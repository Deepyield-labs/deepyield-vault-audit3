...
library RobinhoodStrategyLib {
    ...
    function _redeemMorphoFraction(
        ...
        // The rounding guard above is itself conditioned on a nonzero preview,
        // so an adapter reporting its documented unpriced state (shares held,
        // preview exactly zero) left this cohort redemption with NO floor at
        // all. That is the one moment the floor matters most. A settlement that
        // cannot be priced is deferred, not executed blind: the write-off path
        // exists for a position that is genuinely worthless and carries its own
