...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    function park(
        ...
        uint256 existingShares = shareBalance();
...
library VaultBDepositLib {
    ...
    function instantPricingAssets() external view returns (uint256) {
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
        uint256 liveAssets = vault.totalAssets();
        uint256 referenceAssets = _flowAdjustedNavReference(vault);
        if (referenceAssets == 0 || liveAssets < referenceAssets) return liveAssets;
    ...
    function depositPricingAssetsUpper() external view returns (uint256) {
        ...
        uint256 referenceAssets = _flowAdjustedNavReference(vault);
        return referenceAssets > liveAssets ? referenceAssets : liveAssets;
    }
        ...
        returns (bool deferred)
    {
        ...
        uint256 cutoff = vault.redeemCycleRequestCutoff();
        if (cutoff != 0 && vault.outstandingRedeemShares() < vault.commitThresholdShares() && cutoff < readyAt) {
            readyAt = cutoff;
        }
    ...
    function tryTransfer(address to, uint256 amount) external returns (bool) {
        ...
        // An exact observed debit is authoritative even if a non-canonical token
        // moves value but returns malformed/false data. Never credit escrow twice.
        return beforeBalance >= afterBalance && beforeBalance - afterBalance == amount;
    }
...
library VaultBRedemptionLib {
    ...
    function forceCancelExpiredRedeem(
        ...
        if (request.status != RedeemStatusData.PENDING) revert RedeemRequestUnknown();
        IVaultBRedeemState vault = IVaultBRedeemState(address(this));
        bool deferred = VaultBDepositLib.cancelExpiredWithdrawal(requestId, timeout, localCommitted);
    ...
    {
        RedeemToleranceData storage tolerance = state.redeemTolerances[requestId];
        uint256 referenceAssets = Math.mulDiv(state.redeemCycleAssetsSnapshot, shares, state.redeemCycleSupplySnapshot);
        ...
        returns (bool)
    {
        ...
        uint256 referenceAssets = Math.mulDiv(vault.redeemCycleAssetsSnapshot(), shares, supplySnapshot);
...
library RobinhoodStrategyLib {
    ...
    function _tryVenueLower(address venueAddress) private view returns (bool available, uint256 value) {
        bytes memory data;
        (available, data) =
            venueAddress.staticcall(abi.encodeCall(IRobinhoodVenueValue.estimatedValueLower, ()));
        if (!available || data.length != 32) return (false, 0);
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    function _setInstantNavReference(uint256 assets, uint256 supply) internal {
        if (!_requiresPreSettlement()) return;
        if (supply == 0) {
            instantNavReferenceAssets = 0;
            instantNavReferenceSupply = 0;
            return;
        }
        instantNavReferenceAssets = assets;
        instantNavReferenceSupply = supply;
    }
    ...
    function unpause() external onlyRole(ADMIN_ROLE) {
        ...
        if (pendingTreasury != address(0)) {
            // Treasury replacement cannot be paired with an unpause after its
            // original exit window elapsed while holders could not act.
            // forge-lint: disable-next-line(unsafe-typecast)
            pendingTreasuryReadyAt = uint64(block.timestamp + STRATEGY_TIMELOCK);
        }
        _unpause();
        ...
        returns (uint256 shares)
    {
        ...
        if (shares == 0) revert ZeroAmount();
        _deposit(_msgSender(), receiver, assets, shares);
        if (instantNavReferenceSupply == 0) {
            _setInstantNavReference(totalAssetsUpper(), totalSupply());
        }
    }
    ...
    function claimRedeem(uint256 requestId) external nonReentrant returns (uint256 assets) {
        ...
        uint256 settlementAssets = redeemCycleSettlementAssets;
        uint256 payoutClaimed = redeemCyclePayoutClaimed;
        bool checkpointSettlement = result.committed && redeemCycleSupplySnapshot != 0 && settlementAssets != 0;
        if (result.forceCanceled) {
        ...
        if (outstandingRedeemCount == 0) {
            if (totalSupply() == 0) {
                _setInstantNavReference(0, 0);
            } else if (checkpointSettlement) {
                uint256 remainingAssets = settlementAssets > payoutClaimed ? settlementAssets - payoutClaimed : 0;
                _setInstantNavReference(remainingAssets, totalSupply());
            }
        }
    ...
    ReentrancyGuard
{
    ...
    /// @notice Permissionless completion is safe after guardian authorization:
    /// the adapter burns only the frozen share snapshot and can pay only this
    /// Strategy, which forwards spendable USDG only to the immutable Vault.
