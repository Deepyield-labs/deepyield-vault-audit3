...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// administered by the delayed DEFAULT_ADMIN_ROLE so the canonical Safe can
    /// replace a lost or malicious guardian.
    ...
    constructor(
        ...
        // accept-required transfer. Guardian membership retains AccessControl's
        // DEFAULT_ADMIN_ROLE admin so the canonical Safe has a recovery path.
    ...
    function forceSettleStuckCycle() external nonReentrant {
        ...
        if (!strategyUnavailable && (!paused() || !hasRole(ADMIN_ROLE, msg.sender))) {
...
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    ...
    /// @notice Robinhood Strategy's upper estimate already includes its direct
    /// USDG balance, Morpho shares and tracked Venue assets. Do not add its
    /// pinned source balance a second time.
    function totalAssetsUpper() public view override returns (uint256) {
        return
            VaultBDepositLib.totalAssetsUpperWithoutSource(
                IERC20(asset()), address(this), strategy, totalClaimableAssets
            );
    }

    function commitThresholdShares() public view override returns (uint256 threshold) {
        if (outstandingRedeemCount == 0) return 0;
        threshold = Math.mulDiv(totalSupply(), ROBINHOOD_BATCH_COMMIT_BPS, 10_000, Math.Rounding.Ceil);
