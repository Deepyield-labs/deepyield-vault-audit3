...
contract RobinhoodPriceGuard {
    ...
    function lowerRiskValue(RobinhoodMarket market, uint256 riskAmount) public view returns (uint256) {
        ...
        try this.exitPrices(market) returns (Prices memory normal) {
            ...
            if (emergency.spotUsdGPerRisk < referencePrice) referencePrice = emergency.spotUsdGPerRisk;
            if (emergency.oracleUsdGPerRisk != 0 && emergency.oracleUsdGPerRisk < referencePrice) {
                referencePrice = emergency.oracleUsdGPerRisk;
            }
...
library RobinhoodVenueLib {
    ...
    function recordExecutionLoss(
        ...
        if (window.startedAt == 0 || block.timestamp >= uint256(window.startedAt) + windowLength) {
            ...
            // Exit must always remain possible, but it must never create fresh
            // risk budget for a later LP admission. Start a new window with no
            // admission denominator; the next enforced open supplies its own.
            window.base = 0;
            window.consumed = 0;
        }
        if (enforce) {
            // Every admission stands on its own current notional. A previous
            // large close/withdrawal cannot subsidize repeated smaller opens.
