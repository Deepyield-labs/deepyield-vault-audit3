...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        // accept-required transfer. Guardian membership is deliberately independent.
    ...
    function forceSettleStuckCycle() external nonReentrant {
        if (!_redeemCycleCommitted) {
            (bool committed, bool navAvailable, uint256 assetsSnapshot) = VaultBDepositLib.redeemCycleRecoverySnapshot();
            if (!committed || !navAvailable || outstandingRedeemCount == 0) revert RedeemCycleNotCommitted();
...
contract RobinhoodTreasuryVault is DeepYieldVaultB {
    ...
    function commitThresholdShares() public view override returns (uint256 threshold) {
        if (outstandingRedeemCount == 0) return 0;
        return VaultBDepositLib.redeemCommitThreshold(
            totalSupply(), redeemCycleThresholdBase, MIN_REDEEM_SHARES, ROBINHOOD_BATCH_COMMIT_BPS
        );
    }
