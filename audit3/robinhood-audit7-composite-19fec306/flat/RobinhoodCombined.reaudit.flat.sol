...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    function park(
        ...
        uint256 existingShares = parkedShares;
        ...
        returns (uint256 assetsReceived)
    {
        bytes32 pendingExitId = zeroPreviewEmergencyExitId;
        if (
            pendingExitId != bytes32(0) && block.timestamp <= zeroPreviewExitExpiresAt
                && morphoVault.previewRedeem(zeroPreviewExitShares) == 0
        ) {
            revert ZeroPreviewExitPending(pendingExitId);
        }
        ...
        returns (uint256 assetsReceived)
    {
        ...
        parkedAssets = ownBefore == 0 ? 0 : Math.mulDiv(parkedAssets, ownBefore - ownBurned, ownBefore);
...
library RobinhoodSettlementLib {
    ...
    function claimWithdrawal(
        ...
        if (redemption.cycleCommitted && (!redemption.cycleLiquidityPrepared || !redemption.cyclePayoutFinalized)) {
