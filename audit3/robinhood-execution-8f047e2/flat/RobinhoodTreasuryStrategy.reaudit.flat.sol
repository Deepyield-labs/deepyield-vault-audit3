...
contract RobinhoodPriceGuard {
    ...
    /// @notice Emergency exits retain a bounded spot/TWAP corridor, but use
    /// twice the normal divergence budget so the normal circuit breaker does
    /// not become a permanent unwind lock during a genuine dislocation.
    function emergencyExitPrices(RobinhoodMarket market) public view returns (Prices memory p) {
        return _exitPrices(market, uint256(maxSpotTwapDeviationBps) * 2);
    }
    ...
    function minimumOut(
        ...
        // Execution is always priced from the manipulation-resistant TWAP.
        // Entry performs its separate session/oracle coherence checks before
        // and after the swap; exit deliberately remains available during an
        // oracle pause, stale feed or closed equity session.
...
library RobinhoodVenueLib {
    ...
    function closeReference(
        ...
        referenceTotal = trackedStable + positionStable + guard.twapRiskValue(market, trackedRisk + positionRisk);
    }
    ...
    function executeClose(
        ...
        uint256 swapOutput;
        if (riskToSell != 0) {
            uint256 routerFloor = x.suppliedRouterFloor;
            uint256 requiredFloor =
                guard.minimumOut(x.market, address(risk), address(stable), riskToSell, x.slippage, x.emergency);
            ...
            if (riskSpent != riskToSell || swapOutput != routerOut || swapOutput < routerFloor) {
...
contract BoundedUniswapV3Venue is ReentrancyGuard, IERC721Receiver {
        ...
        returns (uint256 tokenId, uint256 riskAmount, uint256 usdgAmount)
    {
        ...
        activeRiskInventory = riskDesired - riskAmount;
    ...
    function close(CloseParams calldata p) external onlyController nonReentrant returns (uint256 assetsReturned) {
        ...
        (RobinhoodVenueLib.PositionData memory position,,, uint256 referenceTotal) = RobinhoodVenueLib.closeReference(
            positionManager,
            selectedPool,
            guard,
            market,
        ...
        int24 priceLimitReferenceTick = p.emergency ? spotTick : prices.twapTick;
        uint160 priceLimit = RobinhoodVenueLib.priceLimit(
            riskToken0, true, spotTick, priceLimitReferenceTick, maxSwapTickMovement, p.emergency
        );

        assetsReturned = RobinhoodVenueLib.executeClose(
            positionManager,
            router,
            risk,
            usdg,
    ...
    function _estimatedValue(uint8 mode) internal view returns (uint256 value) {
        ...
        if (totalRisk != 0) {
            if (mode == 1) value += guard.upperRiskValue(market, totalRisk);
            else if (mode == 2) value += guard.twapRiskValue(market, totalRisk);
...
contract RobinhoodTreasuryStrategy is IVaultBAsyncStrategy, AccessControlDefaultAdminRules, ReentrancyGuard {
        ...
        returns (uint256 assetsRecovered)
    {
        ...
        bool halted = state == StrategyState.HALTED;
        if (!halted && state != StrategyState.LP_ACTIVE) revert InvalidState(StrategyState.LP_ACTIVE, state);
        if (halted && !guardian) _checkRole(GUARDIAN_ROLE, msg.sender);
        ...
        bytes32 jobId = activeJobId;
        uint256 grossBeforeClose = estimatedGrossAssetsExecution();
        ...
        if (cycleCommitted && grossBeforeClose > grossAfter) cycleExecutionLoss = grossBeforeClose - grossAfter;
        activeJobId = bytes32(0);
        activeMarket = RobinhoodMarket.NONE;
        activeTokenId = 0;
        state = halted || p.emergency ? StrategyState.HALTED : StrategyState.MORPHO_IDLE;
    ...
    function withdrawalCycleChargeableExecutionLoss() external view returns (uint256) {
        (, uint256 feeAfter) = pendingPerformanceFee();
        return RobinhoodStrategyLib.chargeableLoss(cycleExecutionLoss, cycleFeeBefore, feeAfter);
    }
    ...
    function _commitCycle() internal {
        ...
        (, cycleFeeBefore) = pendingPerformanceFee();
