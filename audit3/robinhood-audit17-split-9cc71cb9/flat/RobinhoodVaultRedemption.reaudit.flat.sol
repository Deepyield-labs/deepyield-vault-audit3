...
// NOTE: VaultBDepositLib function bodies (54 functions) are replaced by reverting
// stubs here. The library is reviewed in full in RobinhoodVaultLibraries.reaudit.flat.sol. Every function of
// it is external, so each call from the contract above is a delegatecall to
// the linked address: the caller's deployed runtime is byte-identical with or
// without these bodies, which this package's ABI/runtime equivalence check
// verifies against the project build. Types, errors and events are kept.
...
// NOTE: VaultBRedemptionLib function bodies (34 functions) are replaced by reverting
// stubs here. The library is reviewed in full in RobinhoodVaultLibraries.reaudit.flat.sol. Every function of
// it is external, so each call from the contract above is a delegatecall to
// the linked address: the caller's deployed runtime is byte-identical with or
// without these bodies, which this package's ABI/runtime equivalence check
...
library VaultBRedemptionLib {
    ...
    function _settlementCohortBasis( uint256 settlementAssets, uint256 chargeableLoss, uint256 committedShares, uint256 supplySnapshot ) private pure returns (uint256) {
        revert();
    }
