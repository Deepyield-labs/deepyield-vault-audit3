...
library VaultBDepositLib {
    ...
    function activateCandidate(
        ...
        if (address(oldStrategy) != address(0)) {
            ...
            if (!attested && !responsive && !emergencyAllowed) revert StrategyNotEmpty();
        ...
        asset.forceApprove(address(candidate), type(uint256).max);
    }
    ...
    function validateRedeemRequest(uint256 minimumShares, uint256 newShares) external pure {
    ...
    function cancelExpiredWithdrawal(
        ...
        uint256 timeout,
        bool localCommitted,
        bool maturedMigrationRecovery
    ) external returns (bool deferred) {
        uint256 readyAt = uint256(requestedAt) + timeout;
        if (!maturedMigrationRecovery && block.timestamp < readyAt) {
    ...
    function redeemCycleRecoverySnapshot() external view returns (bool committed, uint256 assetsSnapshot) {
        ...
        if (strategyResponsive && strategyCommitted != 0) {
            (, assetsSnapshot) = _recoverySnapshotAssets(asset, address(p), strategy, reserved);
            return (true, assetsSnapshot);
        }
        (bool directResponsive, uint256 directCommitted) =
            _staticUint(assetSource, IVaultBDirectWithdrawalCycle.withdrawalCycleCommitted.selector, COMMIT_PROBE_GAS);
        if (!directResponsive || directCommitted == 0) return (false, 0);
        (, assetsSnapshot) = _recoverySnapshotAssets(asset, address(p), strategy, reserved);
        return (true, assetsSnapshot);
    }
    ...
    function strategyRecoveryStatus(IVaultBAsyncStrategy strategy, uint256 probeGas, uint256 recoveryGas)
        external
        view
        returns (bool unavailable, bool settlementHealthy)
    {
        _requireGas(probeGas + recoveryGas);
        (bool committedResponsive, uint256 committed) =
            _staticUint(address(strategy), IVaultBAsyncStrategy.withdrawalCycleCommitted.selector, probeGas);
        if (!committedResponsive) return (true, false);
        return (false, committed != 0);
    }
        ...
        returns (uint256)
    {
        ...
        bool committed = localCommitted || activeStrategy.withdrawalCycleCommitted();
...
contract DeepYieldVaultB is ERC4626, AccessControlDefaultAdminRules, Pausable, ReentrancyGuard {
    ...
    /// ADMIN_ROLE grants remain ordinary AccessControl operations. GUARDIAN_ROLE is
    /// self-administered, so default-admin compromise cannot acquire emergency power.
    ...
    /// Admission is economic and does not depend on this value: each new owner seat
    /// must contribute MIN_REDEEM_SHARES, and commit remains an explicit 5% action.
    ...
    uint256 internal constant REDEEM_REQUEST_TIMEOUT = 30 days;
    ...
    constructor(
        ...
        // accept-required transfer. Guardian membership is self-administered so the
        // default admin can never escalate into emergency authority.
    ...
    /// Accordingly previewDeposit/previewMint intentionally revert during a strategy
    /// outage instead of publishing a fabricated share price; this is a documented
    /// availability deviation from the otherwise canonical ERC-4626 preview surface.
    ...
    function applyStrategy() external onlyRole(ADMIN_ROLE) nonReentrant {
        ...
        bool writeOffApproved = _emergencyStrategySourceWriteOffScheduled && block.timestamp >= pendingStrategyReadyAt;
        _activateStrategy(next, _pendingStrategyAssetSource, paused(), writeOffApproved);
        _clearStrategyProposal();
    }

    /// @notice A nonempty, unresponsive old custody source may be abandoned only
    /// after an independent guardian decision and a second full strategy delay.
    ...
    /// @notice Tune legacy queue monitoring bounds without changing admission or
    /// commitment. Retained so existing governance tooling and storage remain stable.
    function setMaxPendingRedeems(uint256 newMax) external onlyRole(ADMIN_ROLE) {
        ...
        // Keep monitoring configuration coherent with the value recorded at queue open.
        if (outstandingRedeemCount > 0 && newMax < redeemCycleMaxPendingAtOpen) {
            revert InvalidMaxPendingRedeems(newMax);
        }
    ...
    function availableImmediateLiquidity() public view returns (uint256) {
        return
            VaultBDepositLib.availableImmediateLiquidity(IERC20(asset()), address(this), strategy, totalClaimableAssets);
    }
        ...
        returns (uint256 requestId)
    {
        ...
        VaultBDepositLib.validateRedeemRequest(MIN_REDEEM_SHARES, shares);
    ...
    /// idle reserve and cannot trigger a vault-wide cycle.
    ...
    /// cycle requires the guardian to pause first, then explicitly settle it.
    /// Recovery deliberately does not price the batch: every remaining request
    /// becomes a zero-asset cancellation that returns its escrowed shares.
    function forceSettleStuckCycle() external nonReentrant {
        if (!_redeemCycleCommitted) {
            (bool committed, uint256 assetsSnapshot) = VaultBDepositLib.redeemCycleRecoverySnapshot();
            if (!committed || outstandingRedeemCount == 0) revert RedeemCycleNotCommitted();
            _writeRedeemCycleSnapshot(0, assetsSnapshot);
            return;
        }
        ...
        (bool strategyUnavailable,) = VaultBDepositLib.strategyRecoveryStatus(
            activeStrategy, FORCE_SETTLE_PROBE_GAS, MIN_FORCE_SETTLE_GAS_AFTER_PROBE
        );
        if (!strategyUnavailable && (!paused() || !hasRole(GUARDIAN_ROLE, msg.sender))) {
    ...
    function _takeRedeemCycleSnapshot(uint256 threshold) internal {
        _writeRedeemCycleSnapshot(threshold, totalAssets());
    }
    ...
    function claimRedeem(uint256 requestId) external nonReentrant returns (uint256 assets) {
        ...
        IVaultBAsyncStrategy activeStrategy = strategy;
        if (address(activeStrategy) == address(0)) revert StrategyUnset();
        // B11-T3: a force-canceled cycle has no asset payout, so it bypasses the
        // broken readiness gate and releases the canonical zero-asset request.
        if (
        ...
        bool forceCanceled = redeemCycleForceSettled;
    ...
    function forceCancelExpiredRedeem(uint256 requestId) external nonReentrant {
        ...
        bool deferred = VaultBDepositLib.cancelExpiredWithdrawal(
            ...
            paused() && pendingStrategy != address(0) && block.timestamp >= pendingStrategyReadyAt
        );
        if (deferred) {
