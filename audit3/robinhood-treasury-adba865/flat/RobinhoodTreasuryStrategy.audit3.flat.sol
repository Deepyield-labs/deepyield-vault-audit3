...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    function park(
        ...
        uint256 residual = asset.balanceOf(address(this));
...
contract RobinhoodTreasuryStrategy is IVaultBAsyncStrategy, AccessControlDefaultAdminRules, ReentrancyGuard {
    ...
    struct GuardConfig {
        ...
        uint16 maxOracleTwapDeviationBps;
        uint16 maxNormalSlippageBps;
        uint16 maxEmergencySlippageBps;
    ...
    bytes32 public activeJobId;
    RobinhoodMarket public activeMarket;
    uint64 public activeEligibleSince;
    uint256 public activeTokenId;
    mapping(bytes32 jobId => bool used) public usedJobs;
    ...
    function submitIntent(OpenIntent calldata intent) external onlyRole(FETCHER_ROLE) {
        ...
        if (
            intent.jobId == bytes32(0) || usedJobs[intent.jobId]
                || intent.configHash != venue.marketConfigHash(intent.market) || intent.eligibleSince > block.timestamp
                || intent.validUntil <= block.timestamp
                || intent.validUntil - intent.eligibleSince > MAX_INTENT_LIFETIME || intent.allocationBps == 0
                || intent.allocationBps > maxLpAllocationBps || intent.tickLower >= intent.tickUpper
                || intent.amountOutMinimum == 0
        ) revert InvalidIntent();
        pendingIntents[intent.market] = intent;
        emit IntentSubmitted(intent.market, intent.jobId, intent.eligibleSince, intent.validUntil);
    }
        ...
        returns (uint256 tokenId)
    {
        ...
        OpenIntent storage accepted = pendingIntents[winner];
        if (_intentHash(intent) != _intentHash(accepted)) revert InvalidIntent();
        uint256 readyAt = uint256(intent.eligibleSince) + OPEN_ARBITRATION_DELAY;
        if (block.timestamp < readyAt) revert ArbitrationPending(readyAt);
        if (usedJobs[intent.jobId]) revert JobAlreadyUsed(intent.jobId);

        uint256 gross = estimatedGrossAssetsLower();
        uint256 allocationAssets = Math.mulDiv(gross, intent.allocationBps, BPS);
        ...
        uint256 available = _ensureUsdG(allocationAssets, intent.validUntil, false);
        ...
        BoundedUniswapV3Venue.OpenParams memory venueIntent = BoundedUniswapV3Venue.OpenParams({
            ...
            eligibleSince: intent.eligibleSince,
            validUntil: intent.validUntil,
            tickLower: intent.tickLower,
            tickUpper: intent.tickUpper,
            allocationBps: intent.allocationBps,
            amountOutMinimum: intent.amountOutMinimum
        });
        asset.forceApprove(address(venue), allocationAssets);
        (tokenId,,) = venue.open(venueIntent, allocationAssets);
        asset.forceApprove(address(venue), 0);
        uint256 remainingAllowance = asset.allowance(address(this), address(venue));
        if (remainingAllowance != 0) revert TransferMismatch(0, remainingAllowance);

        delete pendingIntents[winner];
        usedJobs[intent.jobId] = true;
        activeMarket = winner;
        activeJobId = intent.jobId;
        activeEligibleSince = intent.eligibleSince;
        activeTokenId = tokenId;
        activeJobNonce += 1;
        state = StrategyState.LP_ACTIVE;
        emit JobOpened(winner, intent.jobId, activeJobNonce, tokenId, allocationAssets);
    }
    ...
    function _intentHash(OpenIntent memory intent) internal pure returns (bytes32) {
    ...
    function pendingPerformanceFee() public view returns (uint256 profit, uint256 feeAssets) {
        ...
        uint256 shareholderGross = gross > unremittedFee ? gross - unremittedFee : 0;
        if (shareholderGross <= accountedAssets) return (0, 0);
        profit = shareholderGross - accountedAssets;
    ...
    function _commitCycle() internal {
        cycleCommitted = true;
        cycleAssetsBefore = estimatedGrossAssetsLower();
    ...
    function _reduceAccountedAssets(uint256 withdrawn) internal {
        if (withdrawn >= accountedAssets) accountedAssets = 0;
        else accountedAssets -= withdrawn;
    }
}
