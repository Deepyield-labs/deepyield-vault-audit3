...
13. **`recordNormalCloseFailure`'s selector is not caller-supplied.** Its only caller is
   `RobinhoodTreasuryStrategy` (one site, gated on `closeResult.recoverableFailure`), and the
   value is read out of the revert data of a real close attempt caught in the same
   transaction (`RobinhoodStrategyLib`, `mload(add(reason, 0x20))`). The catch block *is* the
   proof that a close was attempted. The library flat cannot show this because the Strategy
   is reviewed in its own scope.

14. **Zero LP-unwind bounds cannot be executed.** `bindWithdrawalCycleExit` validates only the
   bounds' lifetime on purpose: the Venue enforces the economics at close time, reverting
   `UnsafeExecutionFloor` when the supplied minimum is below `expected * (1 - slippage)` and
...
Three reductions are applied so each scope stays reviewable on its own. All three are
verified, not asserted: the package's equivalence check compiles every flat standalone and
compares each unit's ABI and **deployed runtime bytecode** against the project build. A
mismatch would fail packaging.

1. **Linked libraries appear as reverting stubs in the contract scopes that call them.**
   Every function of `VaultBDepositLib`, `VaultBRedemptionLib`, `RobinhoodStrategyLib` and
   `RobinhoodSettlementLib` is `external`, so each call is a delegatecall to a linked
   address and the caller's runtime does not contain the library body. Types, errors and
   events are kept. Each library is reviewed in full in its own file — named in the note
   above each stub.
2. **Unreferenced dependency boilerplate is omitted.** `SafeCast` ships 65 conversion
   helpers; two are called here. Unused `internal` functions are never emitted by the
   compiler, so removing them cannot change the runtime.
3. **The Vault's libraries have their own scope** (`RobinhoodVaultLibraries`) rather than
   being folded into the Vault file. Coverage is unchanged; the review is split.

## Scopes

| file | contents |
|---|---|
| `RobinhoodVaultRedemption.reaudit.flat.sol` | vault contract (its libraries are stubbed, see below) |
| `RobinhoodVaultLibraries.reaudit.flat.sol` | the vault's two linked libraries, in full |
