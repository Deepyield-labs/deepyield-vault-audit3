...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
    ...
    {
        bytes32 pendingExitId = zeroPreviewEmergencyExitId;
        (bool pendingPriced, uint256 pendingPreview) = _tryPreviewRedeem(zeroPreviewExitShares);
        if (
            pendingExitId != bytes32(0) && block.timestamp <= zeroPreviewExitExpiresAt
                && (!pendingPriced || pendingPreview == 0)
        ) {
            revert ZeroPreviewExitPending(pendingExitId);
        }
        ...
        returns (bytes32 exitId, uint256 shares, uint64 readyAt, uint64 expiresAt)
    {
        ...
        // Arming requires a proven zero; a reverting preview proves nothing.
