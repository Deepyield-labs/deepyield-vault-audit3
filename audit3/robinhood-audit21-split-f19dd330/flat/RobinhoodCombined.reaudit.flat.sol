...
library RobinhoodStrategyLib {
    ...
    function policyWithdrawLimit(
        ...
        // The caller adds its own spendable idle to whatever this returns
        // (RobinhoodTreasuryVault.availableImmediateLiquidity), so this figure
        // must be the STRATEGY's contribution alone. Including Vault idle here
        // counted it twice and let an instant exit pass a limit it never met.
        if (!lpActive) {
        ...
        // Headroom is system-wide and Vault idle is spent from it first, so only
        // what remains after that may be offered from the Strategy. The caller's
        // own idle plus this figure therefore stays inside the 70/30 ceiling
        // instead of exceeding it by the amount of that idle.
    ...
    ReentrancyGuard
{
        ...
        returns (uint256 assetsReleased)
    {
        ...
        if (activeTokenId != 0 || venue.activeTokenId() != 0) revert StrategyUnavailable();
        return
