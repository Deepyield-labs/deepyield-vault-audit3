# Round 29 current response and cumulative audit history

Frozen source candidate: `0abc8d0e7ee1ace83df14661560f040463edacdd`

Round 28 base: `ccf1815a3ddc7ff2efc9fdbda9d5ef4640dbb167`

Production status: pre-deployment; no deployment, roles, balances or positions
were changed.

Read this section first. The complete earlier response is retained below as
provenance. If historical wording conflicts with Round 29, this section
controls. This is evidence for independent verification, not a request to
suppress findings.

## Job 890 H-1 — exact cohort reserve

The only confirmed Strategy-libraries defect was an inconsistent fee floor
between reserve sizing, fee crystallization and reserve transfer. The checkpoint
now protects the complete immutable-snapshot reserve, including the cohort's
direct USDG leg:

```solidity
_checkpointFeeProtected(
    c.asset, c.morpho, c.venue, c.feeRecipient,
    accounting.unremittedFee, accounting.accountedAssets,
    redemption.performanceFeeBps, realized.reservedToVault
);
```

After crystallization, the remaining fee floor is recomputed only from cash
surplus over that reserve:

```solidity
uint256 surplusAfterReserve = balanceAfterCheckpoint > realized.reservedToVault
    ? balanceAfterCheckpoint - realized.reservedToVault
    : 0;
uint256 postCheckpointFeeFloor = accounting.unremittedFee < surplusAfterReserve
    ? accounting.unremittedFee
    : surplusAfterReserve;
```

Finally, custody transfer cannot silently clip the already calculated reserve:

```solidity
if (transferred != requested) revert TransferMismatch(requested, transferred);
```

Every late fee-sink, Morpho top-up or transfer failure therefore reverts the
entire settlement. It cannot turn into a smaller cohort reserve.

Job 890 H-2 was not a canonical defect and received no source fix. The exact
companion `RobinhoodTreasuryStrategy.closeLp` and
`BoundedUniswapV3Venue.close` bodies are quoted at the end of the Strategy
Libraries flat. They show that a recoverable residual sets
`activePositionBurned = true` and returns before `activeTokenId` is cleared,
and that a later bounded close remains callable during commitment. The reported
terminal state `activeTokenId == 0 && activePositionBurned == true` is therefore
not created by the canonical Venue.

## Job 891 F-1 — one frozen queue bargain

The confirmed queue-seat mismatch is closed without changing the fixed cohort
threshold. The first seat freezes both opening supply and opening capacity;
every later new owner uses the same pair:

```solidity
uint256 maximumSeats = requestCount == 0
    ? vault.maxPendingRedeems()
    : vault.redeemCycleMaxPendingAtOpen();
uint256 basis = requestCount == 0
    ? vault.totalSupply()
    : vault.redeemCycleThresholdBase();
...
Thus a full frozen queue reaches 20% for Robinhood and 5% for generic Vault B.
An instant exit cannot cheapen later seats; a cap increase applies only to the
next cohort; existing-owner aggregation remains available. Final cycle clearing
already resets both frozen fields. No storage slot was added.

The report's other canonical reachability premises were not patched. The Vault
Libraries flat now quotes the decisive companion bodies directly: ordinary
`withdraw`/`redeem` reject a committed cycle; uncommitted
`forceSettleStuckCycle` requires maturity plus a positive external commitment;
the canonical Strategy's `depositAssetSource()` returns itself; and Robinhood
claim/settlement wrappers require pre-settlement. These quoted bodies are
compile-inert context and do not expand paid scope.

## Round 29 evidence

- Complete Robinhood matrix: **1000 PASS / 0 FAIL / 0 SKIP**, 17 suites, `-j 2`.
- New reserve regressions: 3/3 PASS; new queue-capacity regressions: 8/8 PASS.
- Queue-focused successor matrix: 553/553 PASS.
- ABI, structural storage and linked-library shape against Round 28: unchanged.
- All seven flats were rebuilt and independently compiled standalone against
  this exact clean source candidate; detailed results follow in
  `EQUIVALENCE_RAW.json`.
- No archive-RPC fork PASS is claimed.

Only Strategy Libraries and Vault Libraries changed materially and are intended
as separate paid re-audit scopes. The remaining five files preserve a coherent,
independently verifiable graph.

---

# Prior Round 28 response and cumulative audit history
