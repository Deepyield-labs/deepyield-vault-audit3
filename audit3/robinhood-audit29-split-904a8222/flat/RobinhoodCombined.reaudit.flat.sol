...
library RobinhoodStrategyLib {
    ...
    function _policyHeadroom(
        ...
        uint256 minimumPostExitNav = Math.mulDiv(
            IRobinhoodVenueValue(venueAddress).estimatedValueUpper(), BPS, maximumLpBps, Math.Rounding.Ceil
        );
    ...
    function transferCycleReserve(
        ...
        // The Vault prices the cohort from its immutable cycle snapshot and
        // requires this reserve in full. Returning a clipped amount merely
        // postpones the failure to the Vault callback and obscures the broken
        // settlement invariant; fail atomically at the custody boundary.
        if (transferred != requested) revert TransferMismatch(requested, transferred);
    ...
    function crystallizeFee(
        ...
        if (totalOwed != 0) {
            ...
            uint256 available =
                _ensureUsdGBestEffort(asset, morphoAddress, totalOwed, protectedBalance, block.timestamp);
        ...
        returns (bool priced, uint256 gross)
    {
        return _tryGrossAssets(
            ...
            venueAddress,
            upper
                ? IRobinhoodVenueValue.estimatedValueUpper.selector
                : IRobinhoodVenueValue.estimatedValueLower.selector
        );
    }
...
library RobinhoodSettlementLib {
    ...
    function settleWithdrawalCycle(
        ...
        (bool grossPriced, uint256 lowerGross) = RobinhoodStrategyLib.tryGrossAssets(c.asset, c.morpho, c.venue, false);
        ...
        (accounting.accountedAssets, accounting.unremittedFee) = _checkpointFeeProtected(
            ...
            accounting.accountedAssets,
            redemption.performanceFeeBps,
            // Protect the complete cohort reserve, including its share of
            // pre-existing direct USDG. Protecting only newly realized sleeves
            // lets fee crystallization consume that direct leg before transfer.
            realized.reservedToVault
        );

        // Re-derive the remaining fee floor after crystallization. Reusing the
        // pre-checkpoint floor double-reserves a liability that may already
        // have been paid, while an uncapped replacement can again suppress the
        // cohort reserve. The exact reserve stays senior; only surplus cash can
        // back any fee that remains unremitted.
        uint256 balanceAfterCheckpoint = c.asset.balanceOf(address(this));
        uint256 surplusAfterReserve =
            balanceAfterCheckpoint > realized.reservedToVault ? balanceAfterCheckpoint - realized.reservedToVault : 0;
        uint256 postCheckpointFeeFloor =
            accounting.unremittedFee < surplusAfterReserve ? accounting.unremittedFee : surplusAfterReserve;
        realized.reservedToVault = RobinhoodStrategyLib.transferCycleReserve(
            c.asset, c.morpho, c.vault, realized.reservedToVault, postCheckpointFeeFloor
        );
    ...
    function _checkpointFee(
        ...
        (nextBasis, nextUnremitted,,) = _checkpointFeeWithAmounts(
            asset, morpho, venue, feeRecipient, oldUnremitted, oldBasis, feeBps, 0
        );
    }
