...
contract RobinhoodPriceGuard {
    ...
    function emergencyExitPrices(RobinhoodMarket market) public view returns (Prices memory p) {
        ...
        try this.twapTick(market) returns (int24 twapTick_) {
            ...
            p.twapUsdGPerRisk = _quoteRiskToUsdG(market, twapSqrt, ONE_RISK_TOKEN);
            independentReference = true;
        } catch {}
        try this.oraclePrice(market) returns (uint256 oraclePrice_) {
            p.oracleUsdGPerRisk = oraclePrice_;
            if (!independentReference || oraclePrice_ < p.twapUsdGPerRisk) p.twapUsdGPerRisk = oraclePrice_;
        } catch {}
    ...
    function upperRiskValue(RobinhoodMarket market, uint256 riskAmount) public view returns (uint256) {
        IRobinhoodV3Pool selectedPool = marketPool(market);
        (uint160 spotSqrt,,,,,,) = selectedPool.slot0();
        uint256 referencePrice = _quoteRiskToUsdG(market, spotSqrt, ONE_RISK_TOKEN);
        try this.twapSqrtPriceX96(market) returns (uint160 twapSqrt) {
            ...
            if (twapPrice > referencePrice) referencePrice = twapPrice;
        } catch {}
        try this.oraclePrice(market) returns (uint256 oraclePrice_) {
            if (oraclePrice_ > referencePrice) referencePrice = oraclePrice_;
        } catch {}
...
contract BoundedMorphoV2Adapter is ReentrancyGuard {
        ...
        returns (uint256 assetsReceived)
    {
        ...
        if (minAssetsOut == 0 && preview != 0) revert InvalidAmount();
...
library RobinhoodVenueLib {
    ...
    function closeRiskPrice(uint256 spotPrice, uint256 twapPrice, bool emergency) external pure returns (uint256) {
        return emergency ? twapPrice : (spotPrice < twapPrice ? spotPrice : twapPrice);
    }
...
contract BoundedUniswapV3Venue is ReentrancyGuard, IERC721Receiver {
    ...
    function close(CloseParams calldata p) external onlyController nonReentrant returns (uint256 assetsReturned) {
        ...
        uint256 referenceRiskPrice =
            RobinhoodVenueLib.closeRiskPrice(prices.spotUsdGPerRisk, prices.twapUsdGPerRisk, p.emergency);
        ...
        if (riskRemaining != 0) {
            if (RobinhoodVenueLib.residualHasRecoverableValue(riskRemaining, referenceRiskPrice, slippage)) {
    ...
    function _estimatedValue(uint8 mode) internal view returns (uint256 value) {
        ...
        RobinhoodPriceGuard.Prices memory prices = mode == 1
            ? guard.upperValuationPrices(market)
...
library RobinhoodStrategyLib {
    ...
    function normalExitAvailable(RobinhoodPriceGuard guard, RobinhoodMarket market)
        external
        view
        returns (bool healthy)
    {
        try guard.exitPrices(market) returns (RobinhoodPriceGuard.Prices memory) {
            healthy = true;
        } catch {}
    }
...
contract RobinhoodTreasuryStrategy is IVaultBAsyncStrategy, AccessControlDefaultAdminRules, ReentrancyGuard {
        ...
        returns (uint256 assetsRecovered)
    {
        ...
        if (p.emergency) {
            ...
            if (RobinhoodStrategyLib.normalExitAvailable(priceGuard, market)) revert EmergencyNotRequired();
    ...
    function _commitCycle() internal {
        ...
        if (state == StrategyState.EXITING) _exitGrossBefore = cycleAssetsBefore;
