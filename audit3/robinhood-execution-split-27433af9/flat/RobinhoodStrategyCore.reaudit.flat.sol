...
/// @dev Audit-only ABI projection; see the separate exact Morpho scope.
...
/// @dev Audit-only ABI projection. The exact production implementation is
/// reviewed in the separate Venue scope; Strategy bytecode equivalence is
/// checked after flattening this projection.
interface BoundedUniswapV3Venue {
    ...
    function intentExecutable(RobinhoodMarket market, int24 tickLower, int24 tickUpper)
        external
        view
        returns (bool);
    function marketConfigHash(RobinhoodMarket market) external view returns (bytes32);
    function open(OpenParams memory p, uint256 allocationAssets)
        external
...
/// @dev Audit-only ABI projection; see the separate exact Oracle scope.
