...
library RobinhoodSettlementLib {
    ...
    function finalizeWithdrawalCycleReserve(
        ...
        if (redemption.cycleBasisUnderwater) {
            ...
            // Reserved cash that was never paid out did not leave the system:
            // it stays in Vault idle, owned by the holders who remain. It
            // therefore cannot reduce their fee high-water mark. The exiting
            // cohort takes its pro-rata share of basis and no more.
            //
            // The previous bound (surplus <= the cohort's own pro-rata figure)
            // was insufficient: it left the snap-to-full-basis branch reachable
            // for any cohort at or above half of supply, because the snap
            // compares the surplus against accountedAssets*(1-f) while the bound
            // only held it under accountedAssets*f. A 60% cohort could wipe the
            // remaining holders' entire high-water mark.
            uint256 survivingFloor =
                accounting.accountedAssets > basisReduction ? accounting.accountedAssets - basisReduction : 0;
            uint256 room = survivingFloor;
            if (reserveSurplus > room) reserveSurplus = room;
            basisReduction += reserveSurplus;
            if (basisReduction > accounting.accountedAssets) basisReduction = accounting.accountedAssets;
        }
        accounting.accountedAssets =
